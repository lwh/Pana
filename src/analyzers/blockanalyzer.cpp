...
{
    changeTimeout( Pana::config( "General" )->readNumEntry( "Timeout", 20 ) );
...
{
    Pana::config( "General" )->writeEntry( "Timeout", timeout() );
}
...
BlockAnalyzer::contextMenuEvent( QContextMenuEvent *e )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "visualizations" ) ), i18n("&Visualizations"),
            0 );
#endif
    ...
    if( id == 0 )
        Pana::Menu::instance()->slotActivated( Pana::Menu::ID_SHOW_VIS_SELECTOR );
    else if( id != -1 ) {
        changeTimeout( id );
        determineStep();
    }
