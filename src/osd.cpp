...
OSDWidget::render( const uint M, const QSize &size )
{
    ...
    if( !m_cover.isNull() )
    {
        ...
        if( !m_scaledCover.hasAlpha() && m_drawShadow &&
          ( m_scaledCover.width() > 22 || m_scaledCover.height() > 22 ) ) {
            // don't draw a shadow for eg, the Pana icon
            QImage shadow;
            ...
            if ( QFile::exists( folder + file ) )
                ...
                shadow.load( locate( "data", "pana/images/shadow_albumcover.png" ) );
...
OSDWidget::event( QEvent *e )
{
    ...
    case QEvent::ApplicationPaletteChange:
        if( !PanaConfig::osdUseCustomColors() )
...
namespace Pana
{
    QImage icon() { return QImage( KIconLoader().iconPath( "pana", -KIcon::SizeHuge ) ); }
}
...
void
Pana::OSD::show( const MetaBundle &bundle ) //slot
{
#ifdef Q_WS_X11
    QString text = "";
    ...
    else
    {
        ...
        if( PanaConfig::osdUsePlaylistColumns() )
        {
            QString tag;
        ...
        else
        {
            ...
            QStringx osd = PanaConfig::osdText();

            // hacky, but works...
            if( osd.contains( "%rating" ) )
                OSDWidget::setRating( PanaConfig::useRatings() ? bundle.rating() : 0 );
            else
                OSDWidget::setRating( 0 );

            osd.replace( "%rating", "" );

            if( osd.contains( "%moodbar" )  &&  PanaConfig::showMoodbar() )
                OSDWidget::setMoodbar( bundle );
            osd.replace( "%moodbar", "" );
...
void
Pana::OSD::applySettings()
{
    setAlignment( static_cast<OSDWidget::Alignment>( PanaConfig::osdAlignment() ) );
    setDuration( PanaConfig::osdDuration() );
#ifdef Q_WS_X11
    setEnabled( PanaConfig::osdEnabled() );
#else
    setEnabled( false );
#endif
    setOffset( PanaConfig::osdYOffset() );
    setScreen( PanaConfig::osdScreen() );
    setFont( PanaConfig::osdFont() );
    setDrawShadow( PanaConfig::osdDrawShadow() );
    setTranslucency( PanaConfig::osdUseFakeTranslucency() );

    if( PanaConfig::osdUseCustomColors() )
    {
        setTextColor( PanaConfig::osdTextColor() );
        setBackgroundColor( PanaConfig::osdBackgroundColor() );
    }
    else unsetColors();
}

void
Pana::OSD::forceToggleOSD()
{
#ifdef Q_WS_X11
    if ( !isShown() ) {
...
void
Pana::OSD::slotCoverChanged( const QString &artist, const QString &album )
{
    if( PanaConfig::osdCover() && artist == EngineController::instance()->bundle().artist()
...
void
Pana::OSD::slotImageChanged( const QString &remoteURL )
{
