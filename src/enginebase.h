...
#ifndef PANA_ENGINEBASE_H
#define PANA_ENGINEBASE_H
 ...
 *
 * The only key thing to get right is what to return from state(), as some Pana
 ...
 *
 * Not returning idle when you have reached End-Of-Stream but Pana has not told you
 ...
 * Pana will expect you to be able to play the URL until stop() or another load() is
 * called.
 *
 * You must handle your own media, do not rely on Pana to call stop() before play() etc.
 *
 * At this time, emitting stateChanged( Engine::Idle ) is not necessary, otherwise you should
 * let Pana know of state changes so it updates the UI correctly.
 *
 * Basically, reimplement everything virtual and ensure you emit stateChanged() correctly,
 * try not to block in any function that is called by Pana, try to keep the user informed
...
namespace Engine
{
    ...
    class LIBPANA_EXPORT Base : public QObject, public Pana::Plugin
    {
    Q_OBJECT
    ...
    signals:
        ...
        /** Shows Pana config dialog at specified page */
    ...
    public:
        ...
        /**
         * Load new track and start Playback. Convenience function for Pana to use.
         * @param url URL to be played.
         * @param stream True if URL is a stream.
         * @return True for success.
         */
        ...
        /** 
         * Whether pana_proxy.rb is needed for last.fm files.
    ...
    protected:
        ...
        /** Shows the Pana configuration dialog at the engine page */
