...
#include "pana.h"         //APP_VERSION, actioncollection
#include "panaconfig.h"   //last.fm username and passwd
...
///////////////////////////////////////////////////////////////////////////////
// CLASS PanaHttp
// PanaHttp is a hack written so that lastfm code could easily use something proxy aware.
// DO NOT use this class for anything else, use KIO directly instead.
////////////////////////////////////////////////////////////////////////////////
PanaHttp::PanaHttp ( const QString& hostname, Q_UINT16 port,
                         QObject* parent )
    : QObject( parent ),
      m_hostname( hostname ),
      m_port( port )
...
int
PanaHttp::get ( const QString & path )
{
...
void
PanaHttp::slotData(KIO::Job*, const QByteArray& data)
{
    if( data.size() == 0 ) {
        return;
    }
...
void
PanaHttp::slotResult(KIO::Job* job)
{
    bool err = job->error();
    ...
    , m_service( 0 )
{
    ...
    m_actionList.append( new KAction( i18n( "Ban" ), Pana::icon( "remove" ),
                         KKey( Qt::CTRL | Qt::Key_B ), this, SLOT( ban() ), ac, "ban" ) );

    m_actionList.append( new KAction( i18n( "Love" ), Pana::icon( "love" ),
                         KKey( Qt::CTRL | Qt::Key_L ), this, SLOT( love() ), ac, "love" ) );

    m_actionList.append( new KAction( i18n( "Skip" ), Pana::icon( "next" ),
...
Controller::getNewProxy( QString genreUrl, bool useProxy )
{
    ...
    if( checkCredentials() )
    {
        ...
        QString pass = PanaConfig::scrobblerPassword();
            ...
            m_service->handshake( user, pass ) )
        {
            ...
            {
                if( !PanaConfig::submitPlayedSongs() )
...
{
    if( PanaConfig::scrobblerUsername().isEmpty() || PanaConfig::scrobblerPassword().isEmpty() )
    {
...
//     bool banned = parameter( "banned", result ) == "1";
    ...
    if ( m_session.lower() == "failed" ) {
        ...
        "Pana failed to establish a session with last.fm. <br>"
        "Check if your last.fm user and password are correctly set."
        ) );
        return false;
    }

    Pana::config( "Scrobbler" )->writeEntry( "Subscriber", m_subscriber );
    if( m_useProxy )
    {
        // Find free port
        ...
        m_server = new Pana::ProcIO();
        m_server->setComm( KProcess::Communication( KProcess::AllOutput ) );
        *m_server << "pana_proxy.rb";
        *m_server << "--lastfm";
        *m_server << QString::number( port );
        *m_server << m_streamUrl.toString();
        *m_server << PanaConfig::soundSystem();
        *m_server << Pana::proxyForUrl( m_streamUrl.toString() );
    
        if( !m_server->start( KProcIO::NotifyOnExit, true ) ) {
            error() << "Failed to start pana_proxy.rb" << endl;
            return false;
        }
        ...
        while( true ) {
            ...
            if( line == "PANA_PROXY: startup" ) break;
        }
...
{
    PanaHttp *http = new PanaHttp( m_baseHost, 80, this );
...
WebService::fetchImageFinished( KIO::Job* job ) //SLOT
{
    ...
    if( job->error() == 0 ) {
        const QString path = Pana::saveLocation() + "lastfm_image.png";
        const int size = PanaConfig::coverPreviewSize();
...
WebService::love() //SLOT
{
    ...
    http->get( QString( m_basePath + "/control.php?session=%1&command=love&debug=%2" )
                  .arg( m_session )
                  .arg( "0" ) );
    Pana::StatusBar::instance()->shortMessage( i18n("love, as in affection", "Loving song...") );
}
...
WebService::skip() //SLOT
{
    ...
    http->get( QString( m_basePath + "/control.php?session=%1&command=skip&debug=%2" )
                  .arg( m_session )
                  .arg( "0" ) );
    Pana::StatusBar::instance()->shortMessage( i18n("Skipping song...") );
}
...
WebService::ban() //SLOT
{
    ...
    http->get( QString( m_basePath + "/control.php?session=%1&command=ban&debug=%2" )
                  .arg( m_session )
                  .arg( "0" ) );
    Pana::StatusBar::instance()->shortMessage( i18n("Ban, as in dislike", "Banning song...") );
}
...
WebService::showError( int code, QString message )
{
    ...
    Pana::StatusBar::instance()->longMessage( message, KDE::StatusBar::Sorry );
}
    ...
    : KDialogBase( parent, "LastfmLogin", true, QString::null, Ok|Cancel)
{
    ...
    new QLabel( i18n( "To use last.fm with Pana, you need a last.fm profile." ), mainWidget() );
...
{
    PanaConfig::setScrobblerUsername( m_userLineEdit->text() );
    PanaConfig::setScrobblerPassword( m_passLineEdit->text() );
