...
#include <pthread.h>//we're emulating features of Qt 4, so this can be removed for pana 2.0
...
ThreadManager::~ThreadManager()
{
    ...
    {
#ifdef HAVE_INOTIFY
        // we don't delete the thread's job as Pana is gone
        // and the Job dtor may expect Pana to be there etc.
...
ThreadManager::Thread::run()
{
    ...
    if ( PanaConfig::databaseEngine().toInt() == DbConnection::sqlite )
