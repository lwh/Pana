...
{
    if( PanaConfig::customRatingsColors() )
        PanaConfig::setCustomRatingsColors( false );
    m_colors[0] = PanaConfig::starColorOne();
    m_colors[1] = PanaConfig::starColorTwo();
    m_colors[2] = PanaConfig::starColorThree();
    m_colors[3] = PanaConfig::starColorFour();
    m_colors[4] = PanaConfig::starColorFive();
    m_halfStarColor = PanaConfig::starColorHalf();
    m_margin = 1;
    m_height = 20;
    reinitStars();
}
...
StarManager::reinitStars( int height, int margin )
{
    ...
    QImage star = QImage( locate( "data", "pana/images/star.png" ) ).smoothScale( hval, hval, QImage::ScaleMin );
    QImage fullStar = QImage( locate( "data", "pana/images/star.png" ) );
    m_star = star.copy();
    ...
    QImage half = QImage( locate( "data", "pana/images/smallstar.png" ) ).smoothScale( hval, hval, QImage::ScaleMin );
    QImage fullHalf = QImage( locate( "data", "pana/images/smallstar.png" ) );
    ...
    for( int i = 0; i < 5; i++ )
    {
        ...
        if( PanaConfig::customRatingsColors() )
        {
            ...
            if( !PanaConfig::fixedHalfStarColor() )
...
StarManager::getHalfStar( int num, bool full )
{
    ...
    else if( PanaConfig::fixedHalfStarColor() || num == -1 )
