 ...
 * copyright (c) 2005 by Seb Ruiz <me@sebruiz.net>
 * copyright (c) 2006 by Alexandre Oliveira <aleprj@gmail.com>
 * copyright (c) 2006 by Martin Ellis <martin.ellis@kdemail.net>
 *
 *                                                                         *
...
////////////////////////////////////////////////////////////////////////////////
// class PanaScriptNewStuff
////////////////////////////////////////////////////////////////////////////////

/**
 * GHNS Customised Download implementation.
 */
class PanaScriptNewStuff : public KNewStuff
{
    public:
    PanaScriptNewStuff(const QString &type, QWidget *parentWidget=0)
        ...
        , m_gui( new ScriptManagerBase( this ) )
{
    ...
    m_generalCategory  ->setPixmap( 0, SmallIcon( Pana::icon( "files" ) ) );
    m_lyricsCategory   ->setPixmap( 0, SmallIcon( Pana::icon( "files" ) ) );
    m_scoreCategory    ->setPixmap( 0, SmallIcon( Pana::icon( "files" ) ) );
    m_transcodeCategory->setPixmap( 0, SmallIcon( Pana::icon( "files" ) ) );

    // Restore the open/closed state of the category items
    KConfig* const config = Pana::config( "ScriptManager" );
    ...
    m_gui->installButton  ->setIconSet( SmallIconSet( Pana::icon( "files" ) ) );
    m_gui->retrieveButton ->setIconSet( SmallIconSet( Pana::icon( "download" ) ) );
    m_gui->uninstallButton->setIconSet( SmallIconSet( Pana::icon( "remove" ) ) );
    m_gui->runButton      ->setIconSet( SmallIconSet( Pana::icon( "play" ) ) );
    m_gui->stopButton     ->setIconSet( SmallIconSet( Pana::icon( "stop" ) ) );
    m_gui->configureButton->setIconSet( SmallIconSet( Pana::icon( "configure" ) ) );
    m_gui->aboutButton    ->setIconSet( SmallIconSet( Pana::icon( "info" ) ) );
...
{
    const QStringList allFiles = kapp->dirs()->findAllResources( "data", "pana/scripts/*", true );
...
ScriptManager::slotInstallScript( const QString& path )
{
    ...
    if( path.isNull() ) {
        _path = KFileDialog::getOpenFileName( QString::null,
            "*.panascript.tar *.panascript.tar.bz2 *.panascript.tar.gz|"
            + i18n( "Script Packages (*.panascript.tar, *.panascript.tar.bz2, *.panascript.tar.gz)" )
            , this
    ...
    QString destination = Pana::saveLocation( "scripts/" );
...
ScriptManager::slotRetrieveScript()
{
    ...
    PanaScriptNewStuff *kns = new PanaScriptNewStuff( "pana/script", this );
    KNS::Engine *engine = new KNS::Engine( kns, "pana/script", this );
    KNS::DownloadDialog *d = new KNS::DownloadDialog( engine, this );
    d->setType( "pana/script" );
    ...
    p->load( "pana/script", "http://amarok.kde.org/knewstuff/amarokscripts-providers.xml" );
...
ScriptManager::slotUninstallScript()
{
    ...
    KURL scriptsDirURL;
    QStringList dirs = KGlobal::dirs()->findDirs( "data", "pana/scripts/" );
...
ScriptManager::slotRunScript( bool silent )
{
    ...
    *script << url.path();
    ...
    if( script->start( KProcess::NotifyOnExit ) )
    {
        ...
        PanaConfig::setLastScoreScript( name );
    }
    else
    {
        if( !silent )
    ...
    li->setPixmap( 0, SmallIcon( Pana::icon( "play" ) ) );
...
ScriptManager::slotAboutScript()
{
    ...
    if( product ) product->setText( i18n( "%1 Pana Script" ).arg( name ) );
...
ScriptManager::slotShowContextMenu( QListViewItem* item, const QPoint& pos )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "clock" ) ), i18n( "Show Output &Log" ), SHOW_LOG );
    menu.insertItem( SmallIconSet( Pana::icon( "edit" ) ), i18n( "&Edit" ), EDIT );
...
ScriptManager::ensureScoreScriptRunning()
{
    ...
    if( runScript( PanaConfig::lastScoreScript(), true /*silent*/ ) )
        return PanaConfig::lastScoreScript();
