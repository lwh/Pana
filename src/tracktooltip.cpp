    ...
    if( force || m_tags != tags || m_tags.url() != tags.url() )
    {
        ...
        for( int i = 0; i < n; ++i )
        {
                ...
                if( rating > 0 )
                {
                    ...
                    for( int i = 0; i < rating / 2; ++i )
                        s += QString( "<img src=\"%1\" height=\"%2\" width=\"%3\">" )
                             .arg( locate( "data", "pana/images/star.png" ) )
                             .arg( QFontMetrics( QToolTip::font() ).height() )
                             .arg( QFontMetrics( QToolTip::font() ).height() );
                    if( rating % 2 )
                        s += QString( "<img src=\"%1\" height=\"%2\" width=\"%3\">" )
                             .arg( locate( "data", "pana/images/smallstar.png" ) )
                             .arg( QFontMetrics( QToolTip::font() ).height() )
                             .arg( QFontMetrics( QToolTip::font() ).height() );
                    right << s;
                    left << playlist->columnText( column );
            ...
            else if( column == PlaylistItem::LastPlayed )
            {
                ...
                right << Pana::verboseTimeSince( lastPlayed );
                left << playlist->columnText( column );
            }
        ...
        m_tooltip += "<center><b>Pana</b></center><table cellpadding='2' cellspacing='2' align='center'><tr>";
...
void TrackToolTip::clear()
{
    m_pos     = 0;
    m_cover = QString::null;
    m_tooltip = i18n( "Pana - rediscover your music player" );
    m_tags    = MetaBundle();
    m_tags.setUrl( KURL() );
