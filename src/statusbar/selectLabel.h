...
#ifndef PANA_SELECTLABEL_H
#define PANA_SELECTLABEL_H
...
class SelectLabel : public QLabel
{
    ...
    Pana::SelectAction const*const m_action;

    signals:
        void activated( int );

    public:
        SelectLabel( Pana::SelectAction const*const action, QWidget *parent )
                : QLabel( parent )
                , m_action( action )
                , m_tooltip( 0 )
    ...
    private:
        void showToolTip()
        {
            ...
            else if( PanaConfig::favorTracks() &&
                     m_action == Pana::actionCollection()->action( "random_mode" ) ) //hack?
            {
                KSelectAction *a = static_cast<KSelectAction*>( Pana::actionCollection()->action( "favor_tracks" ) );
