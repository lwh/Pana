...
KAction *action( const char *name ) { return Pana::actionCollection()->action( name ); }
        ...
        , m_pauseTimer( new QTimer( this ) )
{
    ...
    m_slider = new Pana::PrettySlider( Qt::Horizontal, Pana::PrettySlider::Normal,
					 positionBox );
    ...
    layout->addWidget( new SelectLabel( static_cast<Pana::SelectAction*>( Pana::action( "repeat" ) ), hbox ) );
    layout->addWidget( new SelectLabel( static_cast<Pana::SelectAction*>( Pana::action( "random_mode" ) ), hbox ) );
    layout->addSpacing( 3 );
    ...
    //session stuff
    //setShown( PanaConfig::showStatusBar() );
}
...
StatusBar::engineStateChanged( Engine::State state, Engine::State /*oldState*/ )
{
    ...
    case Engine::Paused:
        m_mainTextLabel->setText( i18n( "Pana is paused" ) ); // display TEMPORARY message
...
StatusBar::drawTimeDisplay( int ms )  //SLOT
{
    ...
    if( PanaConfig::leftTimeDisplayRemaining() && trackLength > 0 )
    {
        seconds2 = seconds;
        seconds = trackLength - seconds;
    // when the left label shows the remaining time and it's a stream
    } else if( PanaConfig::leftTimeDisplayRemaining() && trackLength == 0 )
    {
        seconds2 = seconds;
