...
Playlist::Playlist( QWidget *parent )
        ...
        , m_redoButton( 0 )
        , m_clearButton( 0 )
        , m_undoDir( Pana::saveLocation( "undo/" ) )
        , m_undoCounter( 0 )
        , m_dynamicMode( 0 )
        , m_stopAfterTrack( 0 )
...

{
    ...
    if( PanaConfig::showMoodbar() )
        setColumnWidth( PlaylistItem::Mood, 120 );
    if( PanaConfig::useRatings() )
    ...
    KActionCollection* const ac = Pana::actionCollection();
    ...
    m_clearButton = new KAction( i18n( "clear playlist", "&Clear" ), Pana::icon( "playlist_clear" ), 0, this, SLOT( clear() ), ac, "playlist_clear" );
    m_undoButton  = KStdAction::undo( this, SLOT( undo() ), ac, "playlist_undo" );
    m_redoButton  = KStdAction::redo( this, SLOT( redo() ), ac, "playlist_redo" );
    m_undoButton ->setIcon( Pana::icon( "undo" ) );
    m_redoButton ->setIcon( Pana::icon( "redo" ) );

    new KAction( i18n( "&Repopulate" ), Pana::icon( "playlist_refresh" ), 0, this, SLOT( repopulate() ), ac, "repopulate" );
    new KAction( i18n( "S&huffle" ), "rebuild", CTRL+Key_H, this, SLOT( shuffle() ), ac, "playlist_shuffle" );
    KAction *gotoCurrent = new KAction( i18n( "&Go To Current Track" ), Pana::icon( "music" ), CTRL+Key_J, this, SLOT( showCurrentTrack() ), ac, "playlist_show" );
    new KAction( i18n( "&Remove Duplicate && Dead Entries" ), 0, this, SLOT( removeDuplicates() ), ac, "playlist_remove_duplicates" );
    new KAction( i18n( "&Queue Selected Tracks" ), Pana::icon( "queue_track" ), CTRL+Key_D, this, SLOT( queueSelected() ), ac, "queue_selected" );
    KToggleAction *stopafter = new KToggleAction( i18n( "&Stop Playing After Track" ), Pana::icon( "stop" ), CTRL+ALT+Key_V,
    ...
    m_smartResizing = Pana::config( "PlaylistWindow" )->readBoolEntry( "Smart Resizing", true );
    ...
    Pana::ToolTip::add( this, viewport() );
...
Playlist::~Playlist()
{
    ...
    if( PanaConfig::savePlaylist() && m_lockStack == 0 ) saveXML( defaultPlaylistPath() );

    //speed up quit a little
    safeClear();   //our implementation is slow
    Pana::ToolTip::remove( viewport() );
...
{
    if( list.isEmpty() ) {
        Pana::StatusBar::instance()->shortMessage( i18n("Attempted to insert nothing into playlist.") );
    ...
    if( options & Queue )
    {
        ...
        if ( alreadyOnPlaylist )
        {
            ...
            Pana::StatusBar::instance()->shortMessage(
...
{
    return Pana::saveLocation() + "current.xml";
}
...
Playlist::restoreSession()
{
    ...
    if ( Pana::config()->readBoolEntry( "First 1.4 Run", true ) ) {
        // On first startup of 1.4, we load a special playlist with an intro track
        url.setPath( locate( "data", "pana/data/firstrun.m3u" ) );
        Pana::config()->writeEntry( "First 1.4 Run", false );
    }
    else
        url.setPath( Pana::saveLocation() + "current.xml" );
        ...
        else if( PanaConfig::randomMode() )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Q_INT64 random;
                    if( Pana::favorLastPlay() ) //really big huge numbers
                    {
...
Playlist::playPrevTrack()
{
    ...
    if( Pana::entireAlbums() )
    {
        item = 0;
        if( m_currentTrack )
        {
            item = m_currentTrack->prevInAlbum();
            if( !item && Pana::repeatAlbum() && m_currentTrack->m_album->tracks.count() )
    ...
    else
    {
        ...
        else if( !PanaConfig::randomMode() || m_prevTracks.count() <= 1 )
        {
            if( item )
            {
...
{
    if ( !currentTrack() )
        playNextTrack( Pana::repeatTrack() );
...
void Playlist::toggleStopAfterCurrentTrack()
{
    ...
    if( m_stopAfterTrack == item ) {
        setStopAfterMode( DoNotStop );
        Pana::OSD::instance()->OSDWidget::show( i18n("Stop Playing After Track: Off") );
    }
    else
    {
        setStopAfterItem( item );
        item->setSelected( false );
        item->update();
        Pana::OSD::instance()->OSDWidget::show( i18n("Stop Playing After Track: On") );
    }
...
void Playlist::generateInfo()
{
    ...
    if( Pana::entireAlbums() )
        for( MyIt it( this, MyIt::All ); *it; ++it )
            (*it)->refAlbum();
    m_total = 0;
    if( Pana::entireAlbums() || PanaConfig::favorTracks() )
        for( MyIt it( this, MyIt::Visible ); *it; ++it )
            (*it)->incrementTotals();
}
...
Playlist::activate( QListViewItem *item )
{
    ...
    if( !item )
    {
        ...
        EngineController::instance()->stop();
        setCurrentTrack( 0 );
        Pana::OSD::instance()->OSDWidget::show( i18n("Playlist finished"),
                                            QImage( KIconLoader().iconPath( "pana", -KIcon::SizeHuge ) ) );
        return;
    }
    ...
    {
        Pana::StatusBar::instance()->shortMessage( i18n("Local file does not exist.") );
        return;
    }

    if( dynamicMode() && !Pana::repeatTrack() )
    {
...
QPair<QString, QRect> Playlist::toolTipText( QWidget*, const QPoint &pos ) const
{
        ...
        ( col != PlaylistItem::Rating ) )
    {
        ...
        if( item->isCurrent() )
        {
            ...
            Pana::ToolTip::s_hack = 1; //HACK for precise positioning
        }
...
Playlist::setCurrentTrackPixmap( int state )
{
    ...
    m_currentTrack->setPixmap( m_firstColumn, pixmap.isNull() ? QPixmap() : Pana::getPNG( pixmap ) );
...
Playlist::isTrackAfter() const
{
   ...
   return !currentTrack() && !isEmpty() ||
          ...
          totalTrackCount() > 1 && ( PanaConfig::randomMode() || Pana::repeatPlaylist()
          || Pana::repeatAlbum() && repeatAlbumTrackCount() > 1 );
}
...
Playlist::isTrackBefore() const
{
    ...
    return !isEmpty() &&
           (
               currentTrack() && (currentTrack()->itemAbove() || Pana::repeatPlaylist() && totalTrackCount() > 1)
               ||
               PanaConfig::randomMode() && totalTrackCount() > 1
           );
}
...
{
    Pana::actionCollection()->action( "play" )->setEnabled( !isEmpty() );
    Pana::actionCollection()->action( "prev" )->setEnabled( isTrackBefore() );
    Pana::actionCollection()->action( "next" )->setEnabled( isTrackAfter() );
    Pana::actionCollection()->action( "playlist_clear" )->setEnabled( !isEmpty() );
    Pana::actionCollection()->action( "playlist_show" )->setEnabled( m_currentTrack );
...
Playlist::engineStateChanged( Engine::State state, Engine::State /*oldState*/ )
{
    ...
    {
    case Engine::Playing:
        Pana::actionCollection()->action( "pause" )->setEnabled( true );
        Pana::actionCollection()->action( "stop" )->setEnabled( true );
    ...
    case Engine::Paused:
        Pana::actionCollection()->action( "pause" )->setEnabled( false );
        Pana::actionCollection()->action( "stop" )->setEnabled( true );
    ...
    case Engine::Empty:
        Pana::actionCollection()->action( "pause" )->setEnabled( false );
        Pana::actionCollection()->action( "stop" )->setEnabled( false );
...
Playlist::clear() //SLOT
{
    ...
    Pana::actionCollection()->action( "play" )->setEnabled( false );
    Pana::actionCollection()->action( "prev" )->setEnabled( false );
    Pana::actionCollection()->action( "next" )->setEnabled( false );
    Pana::actionCollection()->action( "playlist_clear" )->setEnabled( false );

    ThreadManager::instance()->abortAllJobsNamed( "TagWriter" );

    // something to bear in mind, if there is any event in the loop
    // that depends on a PlaylistItem, we are about to crash Pana
...
Playlist::setColumnWidth( int col, int width )
{
    ...
    //NOTE  default column sizes are stored in default panarc so that restoreLayout() in ctor will
...
Playlist::eventFilter( QObject *o, QEvent *e )
{
    ...
    if( o == header() && e->type() == QEvent::MouseButtonPress && me->button() == Qt::RightButton )
    {
        ...
        sub.setItemVisible( PlaylistItem::Score, PanaConfig::useScores() );
        sub.setItemVisible( PlaylistItem::Rating, PanaConfig::useRatings() );
        sub.setItemVisible( PlaylistItem::Mood, PanaConfig::showMoodbar() );
        ...
        case SMARTRESIZING:
            ...
            Pana::config( "PlaylistWindow" )->writeEntry( "Smart Resizing", m_smartResizing );
            if ( m_smartResizing )
...
Playlist::saveXML( const QString &path )
{
    ...
    stream << QString( "<playlist product=\"%1\" version=\"%2\"%3>\n" )
              .arg( "Pana" ).arg( Pana::xmlVersion() ).arg( dynamic );
...
Playlist::setDynamicMode( DynamicMode *mode ) //SLOT
{
    ...
    m_dynamicMode = mode;
    if( mode )
        PanaConfig::setLastDynamicMode( mode->title() );
    emit dynamicModeChanged( mode );

    if( mode )
    {
        m_oldRandom = PanaConfig::randomMode();
        m_oldRepeat = PanaConfig::repeat();
    }
    Pana::actionCollection()->action( "random_mode" )->setEnabled( !mode );
    Pana::actionCollection()->action( "repeat" )->setEnabled( !mode  );
    Pana::actionCollection()->action( "playlist_shuffle" )->setEnabled( !mode );
    Pana::actionCollection()->action( "repopulate" )->setEnabled( mode );
    if( prev && mode )
    {
...
Playlist::disableDynamicMode() //SLOT
{
    if( !m_dynamicMode )
        return;
    setDynamicMode( 0 );
    PanaConfig::setRandomMode( m_oldRandom );
    PanaConfig::setRepeat( m_oldRepeat );
    static_cast<KSelectAction*>(Pana::actionCollection()->action( "random_mode" ))->setCurrentItem( m_oldRandom );
    static_cast<KSelectAction*>(Pana::actionCollection()->action( "repeat" ))->setCurrentItem( m_oldRepeat );
}
...
Playlist::copyToClipboard( const QListViewItem *item ) const //SLOT
{
    ...
    if( item )
    {
        ...
        Pana::OSD::instance()->OSDWidget::show( i18n( "Copied: %1" ).arg( text ),
...
Playlist::showContextMenu( QListViewItem *item, const QPoint &p, int col ) //SLOT
{
    ...
    {
        KPopupMenu popup;
        Pana::actionCollection()->action("playlist_save")->plug( &popup );
        Pana::actionCollection()->action("playlist_clear")->plug( &popup );
        DynamicMode *m = 0;
        if(dynamicMode())
             popup.insertItem( SmallIconSet( Pana::icon( "dynamic" ) ), i18n("Repopulate"), REPOPULATE);
        else
        {
            Pana::actionCollection()->action("playlist_shuffle")->plug( &popup );
                m = PlaylistBrowser::instance()->findDynamicModeByTitle( PanaConfig::lastDynamicMode() );
                if( m )
                    popup.insertItem( SmallIconSet( Pana::icon( "dynamic" ) ), i18n("L&oad %1").arg( m->title().replace( '&', "&&" ) ), ENABLEDYNAMIC);
        }
        switch(popup.exec(p))
        {
            case ENABLEDYNAMIC:
...
//         popup.insertTitle(i18n("1 Track", "%n Selected Tracks", itemCount));
    ...
    {
        KActionCollection *ac = Pana::actionCollection();
    ...
    if( !isCurrent || !isPlaying )
        popup.insertItem( SmallIconSet( Pana::icon( "play" ) ), isCurrent && isPlaying
                ? i18n( "&Restart" )
                : i18n( "&Play" ), PLAY );
    if( isCurrent && !isLastFm && isPlaying )
        Pana::actionCollection()->action( "pause" )->plug( &popup );

    // Begin queue entry logic
    popup.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n("&Queue Selected Tracks"), PLAY_NEXT );
    ...
    if( itemCount == 1 )
    {
        if ( !firstQueued )
            ...
            popup.changeItem( PLAY_NEXT, SmallIconSet( Pana::icon( "dequeue_track" ) ), i18n("&Dequeue Track") );
    } else {
        if ( queueToggle )
            ...
            if ( !firstQueued )
                ...
                popup.changeItem( PLAY_NEXT, SmallIconSet( Pana::icon( "dequeue_track" ) ), i18n("&Dequeue Selected Tracks") );
    }
    ...
    if( itemCount == 1 )
    {
        Pana::actionCollection()->action( "stop_after" )->plug( &popup );
        dynamic_cast<KToggleAction *>( Pana::actionCollection()->action( "stop_after" ) )->setChecked( m_stopAfterTrack == item );
    }

    if( isCurrent && itemCount == 1 )
    {
        popup.insertItem( SmallIconSet( Pana::icon( "repeat_track" ) ), i18n( "&Repeat Track" ), REPEAT );
        popup.setItemChecked( REPEAT, Pana::repeatTrack() );
    }
    ...
    if( itemCount > 1 )
    {
        popup.insertItem( SmallIconSet( Pana::icon( "playlist" ) ), i18n("&Set as Playlist (Crop)"), CROP_PLAYLIST );
        popup.insertItem( SmallIconSet( Pana::icon( "save" ) ), i18n("S&ave as Playlist..."), SAVE_PLAYLIST );
    }

    popup.insertItem( SmallIconSet( Pana::icon( "remove_from_playlist" ) ), i18n( "Re&move From Playlist" ), this, SLOT( removeSelectedItems() ), Key_Delete, REMOVE );
    ...
    fileMenu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n("&Delete File...", "&Delete %n Selected Files...", itemCount ), this, SLOT( deleteSelectedFiles() ), SHIFT+Key_Delete, DELETE );
    popup.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n("Manage &Files"), &fileMenu, FILE_MENU );

    if( itemCount == 1 )
        popup.insertItem( SmallIconSet( Pana::icon( "editcopy" ) ), i18n( "&Copy Tags to Clipboard" ), COPY );
    ...
    popup.insertItem( SmallIconSet( Pana::icon( "edit" ) ), (itemCount == 1
            ? i18n( "&Edit Tag '%1'" )
            : i18n( "&Edit '%1' Tag for Selected Tracks" )).arg( tagName ), EDIT );

    popup.insertItem( SmallIconSet( Pana::icon( "info" ) )
        , item->url().isLocalFile() ?
    ...
    case REPEAT:
        // FIXME HACK Accessing PanaConfig::Enum* yields compile errors with GCC 3.3.
        static_cast<KSelectAction*>( Pana::actionCollection()->action( "repeat" ) )
            ->setCurrentItem( Pana::repeatTrack()
                              ? 0 /*PanaConfig::EnumRepeat::Off*/
                              : 1 /*PanaConfig::EnumRepeat::Track*/ );
        break;
...
Playlist::removeItem( PlaylistItem *item, bool multi )
{
    ...
    if( m_currentTrack == item )
    {
        ...
        if( m_nextTracks.isEmpty() && !PanaConfig::randomMode() )
        {
...
Playlist::saveState( QStringList &list )
{
   ...
   {
      QString fileName;
      m_undoCounter %= PanaConfig::undoLevels();
      fileName.setNum( m_undoCounter++ );
      fileName.prepend( m_undoDir.absPath() + '/' );
      fileName.append( ".xml" );

      if ( list.count() >= (uint)PanaConfig::undoLevels() )
      {
...
TagWriter::completeJob()
{
    ...
    case true:
        ...
        Pana::StatusBar::instance()->longMessage( i18n(
