        ...
        , m_coverErrors( 0 )
{
    ...
    foreach( artists )
    {
        ...
        item->setPixmap( 0, SmallIcon( Pana::icon( "artist" ) ) );
    }
    m_artistView->sort();
    ...
    item->setPixmap( 0, SmallIcon( Pana::icon( "album" ) ) );
    ...
    m_fetchButton = new KPushButton( KGuiItem( i18n("Fetch Missing Covers"), Pana::icon( "download" ) ), hbox );
    ...
    resize( Pana::config( "Cover Manager" )->readSizeEntry( "Window Size", &size ) );
...
CoverManager::~CoverManager()
{
    ...
    Pana::config( "Cover Manager" )->writeEntry( "Window Size", size() );
...
void CoverManager::fetchCoversLoop() //SLOT
{
    ...
    if( m_fetchCounter < m_fetchCovers.count() ) {
        ...
        // Wait 1 second, since lastfm caps the number of accesses per client
...
void CoverManager::showCoverMenu( QIconViewItem *item, const QPoint &p ) //SLOT
{
    ...
    if( selected.count() > 1 ) {
        menu.insertItem( SmallIconSet( Pana::icon( "download" ) ), i18n( "&Fetch Selected Covers" ), FETCH );
        menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "Set &Custom Cover for Selected Albums" ), CUSTOM );
        menu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n( "&Unset Selected Covers" ), DELETE );
...
{
    int thumbWidth = PanaConfig::coverPreviewSize();
