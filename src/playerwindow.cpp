...
//simple function for fetching pana images
namespace Pana
{
    ...
    {
        QString file = !filename.endsWith( ".png", false ) ? "pana/images/%1.png" : "pana/images/%1";
    ...
    {
        QString file = !filename.endsWith( ".jpg", false ) ? "pana/images/%1.jpg" : "pana/images/%1";

        return QPixmap( locate( "data", QString( "pana/images/%1.jpg" ).arg( filename ) ), "JPEG" );
    }
}
    ...
    , m_pAnalyzer( 0 )
{
    ...
    if ( PanaConfig::playerPos() != QPoint(-1,-1) )
        move( PanaConfig::playerPos() );
    ...
    { //<Sliders>
        m_pSlider    = new Pana::PrettySlider( Qt::Horizontal,
                                                 Pana::PrettySlider::Pretty, this );
        m_pVolSlider = new Pana::PrettySlider( Qt::Vertical,
                                                 Pana::PrettySlider::Pretty, this,
                                                 Pana::VOLUME_MAX );

        m_pSlider->setGeometry( 4,103, 303, 12 );
        m_pVolSlider->setGeometry( 294,18, 12,79 );
        m_pVolSlider->setValue( PanaConfig::masterVolume() );
...
{
    PanaConfig::setPlayerPos( pos() );
    PanaConfig::setPlaylistWindowEnabled( m_pPlaylistButton->isOn() );
...
void PlayerWidget::setScroll( const QStringList &list )
{
    ...
    if( text.isEmpty() ) text = i18n( "Please report this message to pana@bunnies.net, thanks!" );
         ...
         ++it )
    {
        ...
        x += fm.width( *it );
        p.fillRect( x + 8, separatorYPos, 4, 4, Pana::ColorScheme::Foreground );
        x += separatorWidth;
    }
...
void PlayerWidget::engineStateChanged( Engine::State state, Engine::State /*oldState*/ )
{
    ...
    switch( state )
    {
        case Engine::Empty:
            ...
            setScroll( i18n( "Welcome to Pana" ) );
            update();
            break;
...
void PlayerWidget::timeDisplay( int ms )
{
    ...
    const bool showRemaining = PanaConfig::leftTimeDisplayRemaining() && songLength > 0;
...
{
    ///this function is only used by determinePanaColors()
    int ignore, s, v;
    target.getHsv( &ignore, &s, &v );
    return QColor( hue, s, v, QColor::Hsv );
}

void PlayerWidget::determinePanaColors() //static
{
    int hue, s, v;

    (!PanaConfig::schemeKDE()
        ? PanaConfig::playlistWindowBgColor()
...
void PlayerWidget::setModifiedPalette()
{
    ...
    cg.setColor( QColorGroup::Background, Pana::ColorScheme::Base );
    cg.setColor( QColorGroup::Foreground, Pana::ColorScheme::Text );
...
void PlayerWidget::applySettings()
{
    ...
    QFont phont = font();
    phont.setFamily( PanaConfig::useCustomFonts()
        ? PanaConfig::playerWidgetFont().family()
    ...
    case Engine::Empty:
        m_scrollTextPixmap.fill( Pana::ColorScheme::Base );
        update();
        break;
    default:
...
void PlayerWidget::setMinimalView( bool enable )
{
    ...
    if( enable )
    {
        ...
        PanaConfig::setPlayerWindowMinimalView( true );
    }
    else
    {
        ...
        setFixedSize( 311, 140 );
        PanaConfig::setPlayerWindowMinimalView( false );
    }
    ...
    case QEvent::Hide:
...

        {
            ...
            if( e->spontaneous() ) //the window system caused the event
            {
                ...
                //if the user is on another desktop to Pana, do nothing
                ...
                //we caused Pana to hide, so we should hide the Playlist Window
...
{
    Pana::Menu::instance()->exec( e->globalPos() );
}
...
void PlayerWidget::mousePressEvent( QMouseEvent *e )
{
    ...
    else
    {
        ...
        {
            PanaConfig::setLeftTimeDisplayRemaining( !PanaConfig::leftTimeDisplayRemaining() );
...
{
    PanaConfig::setCurrentAnalyzer( PanaConfig::currentAnalyzer() + increment );
