...
class DummyMediaDevice : public MediaDevice
{
    ...
    virtual bool openDevice( bool silent )
    {
        if( !silent )
        {
            ...
            //Pana::StatusBar::instance()->longMessage( msg, KDE::StatusBar::Sorry );
        }
        return false;
    }
        ...
        , m_quitting( false )
{
    ...
    MediaItem::s_pixUnknown = new QPixmap(iconLoader.loadIcon( Pana::icon( "unknown" ), KIcon::Toolbar, KIcon::SizeSmall ));
    MediaItem::s_pixTrack = new QPixmap(iconLoader.loadIcon( Pana::icon( "playlist" ), KIcon::Toolbar, KIcon::SizeSmall ));
    MediaItem::s_pixFile = new QPixmap(iconLoader.loadIcon( Pana::icon( "sound" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixPodcast = new QPixmap(iconLoader.loadIcon( Pana::icon( "podcast" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixPlaylist = new QPixmap(iconLoader.loadIcon( Pana::icon( "playlist" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixRootItem = new QPixmap(iconLoader.loadIcon( Pana::icon( "files2" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    // history
    // favorites
    // collection
    // folder
    ...
    // cancel
    // sound
    MediaItem::s_pixArtist = new QPixmap(iconLoader.loadIcon( Pana::icon( "personal" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixAlbum = new QPixmap(iconLoader.loadIcon( Pana::icon( "cdrom_unmount" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixInvisible = new QPixmap(iconLoader.loadIcon( Pana::icon( "cancel" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixStale = new QPixmap(iconLoader.loadIcon( Pana::icon( "cancel" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixOrphaned = new QPixmap(iconLoader.loadIcon( Pana::icon( "cancel" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixDirectory = new QPixmap(iconLoader.loadIcon( Pana::icon( "folder" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixTransferBegin = new QPixmap(iconLoader.loadIcon( Pana::icon( "play" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixTransferEnd = new QPixmap(iconLoader.loadIcon( Pana::icon( "stop" ), KIcon::Toolbar, KIcon::SizeSmall ) );
    MediaItem::s_pixTransferFailed = new QPixmap(iconLoader.loadIcon( Pana::icon( "cancel" ), KIcon::Toolbar, KIcon::SizeSmall ) );
   ...
   // m_toolbar->setIconText( KToolBar::IconTextRight, true );
    m_toolbar->insertButton( Pana::icon( "add_playlist" ), CUSTOM, SIGNAL( clicked() ), this, SLOT( customClicked() ), true, "custom" );
    QToolTip::add( m_toolbar->getButton(TRANSFER), i18n( "Transfer tracks to media device" ) );

    m_toolbar->setIconText( KToolBar::IconOnly, false );

    m_toolbar->insertButton( Pana::icon( "configure" ), CONFIGURE, true, i18n("Configure") );
    ...
    m_pluginPanaName["dummy-mediadevice"] = i18n( "Disable" );
    m_pluginName[ i18n( "Do not handle" ) ] = "ignore";
    m_pluginPanaName["ignore"] = i18n( "Do not handle" );
    // query available device plugins
    m_plugins = PluginManager::query( "[X-KDE-Pana-plugintype] == 'mediadevice'" );
    ...
    m_cancelButton = new KPushButton( SmallIconSet( Pana::icon( "cancel" ) ), i18n("Cancel"), m_progressBox );
    ...
    queue()->load( Pana::saveLocation() + "transferlist.xml" );
    queue()->computeSize();
    ...
    for( QMap<QString, Medium*>::Iterator it = mmap.begin();
            ...
            it++ )
    {
        QString handler = Pana::config( "MediaBrowser" )->readEntry( (*it)->id() );
        //debug() << "[MediaBrowser] (*it)->id() = " << (*it)->id() << ", handler = " << handler << endl;
        if( handler.isEmpty() )
        {
            //this should probably never be the case with a manually added device, unless panarc's been messed with
            Pana::config( "MediaBrowser" )->writeEntry( (*it)->id(), "ignore" );
            newflag = true;
    ...
    if ( newflag )
        Pana::StatusBar::instance()->longMessageThreadSafe(
                i18n("Pana has detected new portable media devices.\n"
    ...
    m_haveDevices = false;
    QMap<QString,QString> savedDevices = Pana::config( "MediaBrowser" )->entryMap( "MediaBrowser" );
...
MediaBrowser::removeDevice( MediaDevice *device )
{
    ...
    if( device->isConnected() )
    {
        ...
        else
        {
            ...
            Pana::StatusBar::instance()->longMessage(
...
MediaBrowser::~MediaBrowser()
{
    ...
    queue()->save( Pana::saveLocation() + "transferlist.xml" );
...
MediaBrowser::mediumAdded( const Medium *medium, QString /*name*/, bool /*constructing*/ )
{
    ...
    if( medium )
    {
        QString handler = Pana::config( "MediaBrowser" )->readEntry( medium->id() );
        if( handler.isEmpty() )
        {
...
MediaBrowser::pluginSelected( const Medium *medium, const QString plugin )
{
    ...
    if( !plugin.isEmpty() )
    {
        ...
        Pana::config( "MediaBrowser" )->writeEntry( medium->id(), plugin );
...
MediaBrowser::loadDevicePlugin( const QString &deviceType )
{
    ...
    QString query = "[X-KDE-Pana-plugintype] == 'mediadevice' and [X-KDE-Pana-name] == '%1'";
    Pana::Plugin *plugin = PluginManager::createFromQuery( query.arg( deviceType ) );
...
MediaBrowser::configSelectPlugin( int index )
{
    ...
    {
        PanaConfig::setDeviceType( m_pluginName[m_configPluginCombo->currentText()] );
    }
    else if( currentDevice() )
    {
        KConfig *config = Pana::config( "MediaBrowser" );
    ...
    if( m_pluginName[m_configPluginCombo->currentText()] != currentDevice()->deviceType() )
    {
        ...
        *m_currentDevice = loadDevicePlugin( PanaConfig::deviceType() );
        if( !*m_currentDevice )
        {
            *m_currentDevice = new DummyMediaDevice();
            if( PanaConfig::deviceType() != "dummy-mediadevice" )
            {
                QString msg = i18n( "The requested media device could not be loaded" );
                Pana::StatusBar::instance()->shortMessage( msg );
            }
        }
        dev = currentDevice();
        dev->init( this );
...
MediaDevice::configString( const QString &name, const QString &defValue )
{
    ...
    KConfig *config = Pana::config( configName );
...
MediaDevice::bundlesToSync( const QString &name, const KURL &url )
{
    ...
    {
        Pana::StatusBar::instance()->longMessage( i18n( "Not a playlist file: %1" ).arg( url.path() ),
                KDE::StatusBar::Sorry );
        return bundles;
    }
    ...
    if( playlist.isError() )
    {
        Pana::StatusBar::instance()->longMessage( i18n( "Failed to load playlist: %1" ).arg( url.path() ),
                KDE::StatusBar::Sorry );
        return bundles;
    }
...
{
    KURL url = Pana::mostLocalURL( url2 );
    ...
    {
        KURL::List urls = Pana::recursiveUrlExpand( url );
...
MediaQueue::URLsAdded()
{
    ...
    save( Pana::saveLocation() + "transferlist.xml" );
}
...
MediaDevice::connectDevice( bool silent )
{
    ...
    Pana::StatusBar::instance()->shortMessage( i18n( "Device successfully connected" ) );
...
MediaDevice::disconnectDevice( bool postDisconnectHook )
{
    ...
    if( postDisconnectHook && runPostDisconnectCommand() != 0 )
    {
        ...
        Pana::StatusBar::instance()->shortMessage( i18n( "Device successfully disconnected" ) );
    ...
    for( ; it; it = static_cast<MediaItem *>( it->nextSibling() ) )
    {
        ...
        case MediaItem::TRACK:
            if( !it->parent() || static_cast<MediaItem *>( it->parent() )->type() != MediaItem::PLAYLIST )
            {
                ...
                if( it->ratingChanged() )
                {
                    // copy rating from media device to Pana
                    QString url = CollectionDB::instance()->getURL( *bundle );
...
MediaItem::syncStatsFromPath( const QString &url )
{
    ...
    // copy Pana rating, play count and last played time to device
...
MediaDevice::transferFiles()
{
    ...
    while( next )
    {
                ...
                ++it )
        {
            ...
            {
                Pana::StatusBar::instance()->shortMessage( i18n( "Track already on media device: %1" ).
            ...
            else if( !item ) // the item does not yet exist on the media device
            {
                ...
                {
                    Pana::StatusBar::instance()->shortMessage( i18n( "Track not playable on media device: %1" ).arg( bundle->url().path() ),
                            KDE::StatusBar::Sorry );
                    unplayable += (*it).url();
    ...
    if( unplayable.count() + existing.count() > 0 )
    {
        ...
        Pana::StatusBar::instance()->shortLongMessage( msg, longMsg, KDE::StatusBar::Sorry );
    }
    else if( !msg.isEmpty() )
    {
        Pana::StatusBar::instance()->shortMessage( msg, KDE::StatusBar::Sorry );
    }

    m_parent->updateButtons();
    m_parent->queue()->save( Pana::saveLocation() + "transferlist.xml" );
    m_transferring = false;
...
MediaQueue::save( const QString &path )
{
    ...
    transferlist.setAttribute( "product", "Pana" );
...
MediaQueue::load( const QString& filename )
{
    ...
    if( !d.setContent( stream.read(), &er, &l, &c ) ) { // return error values
        Pana::StatusBar::instance()->longMessageThreadSafe( i18n(
                //TODO add a link to the path to the playlist
                "The XML in the transferlist was invalid. Please report this as a bug to the Pana "
...
MediaQueue::slotShowContextMenu( QListViewItem* item, const QPoint& point, int )
{
    ...
    if( item )
        menu.insertItem( SmallIconSet( Pana::icon( "remove_from_playlist" ) ), i18n( "&Remove From Queue" ), REMOVE_SELECTED );

    menu.insertItem( SmallIconSet( Pana::icon( "playlist_clear" ) ), i18n( "&Clear Queue" ), CLEAR_ALL );
    menu.insertItem( SmallIconSet( Pana::icon( "playlist_refresh" ) ), i18n( "&Start Transfer" ), START_TRANSFER );
