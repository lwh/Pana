...
class Pana::ToolTip::Manager: public QObject
{
public:
    Manager( QObject *parent ): QObject( parent ) { qApp->installEventFilter( this ); }
    virtual ~Manager()
    {
        for( int n = Pana::ToolTip::s_tooltips.count() - 1; n >= 0; --n )
            delete Pana::ToolTip::s_tooltips[n];
    }
    ...
    bool eventFilter( QObject*, QEvent *e )
    {
        switch ( e->type() )
        {
            ...
            case QEvent::FocusOut:
                if( !dynamic_cast<Pana::ToolTip*>( qApp->widgetAt( QCursor::pos(), true ) ) )
...
Pana::ToolTip::Manager* Pana::ToolTip::s_manager = 0;
QPoint Pana::ToolTip::s_pos;
QRect Pana::ToolTip::s_rect;
QString Pana::ToolTip::s_text;
QValueList<Pana::ToolTip*> Pana::ToolTip::s_tooltips;
int Pana::ToolTip::s_hack = 0;

void Pana::ToolTip::add( ToolTipClient *client, QWidget *parent ) //static
{
    if( !s_manager )
        s_manager = new Pana::ToolTip::Manager( qApp );
    new ToolTip( client, parent );
}

void Pana::ToolTip::remove( QWidget *widget ) //static
{
...
void Pana::ToolTip::hideTips() //static
{
...
QString Pana::ToolTip::textFor( QWidget *widget, const QPoint &pos ) //static
{
    for( int i = 0, n = s_tooltips.count(); i < n; ++i )
        if( s_tooltips[i]->QToolTip::parentWidget() == widget )
...
void Pana::ToolTip::updateTip() //static
{
    for( int i = 0, n = s_tooltips.count(); i < n; ++i )
...
Pana::ToolTip::ToolTip( ToolTipClient *client, QWidget *parent )
...
void Pana::ToolTip::drawContents( QPainter *painter )
{
...
QSize Pana::ToolTip::sizeHint() const
{
    if( !s_rect.isNull() )
        return s_rect.size();
    else
