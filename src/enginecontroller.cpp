...
EngineController::loadEngine() //static
{
    ...
    if( m_engine != m_voidEngine ) {
        ...
        // we need to do it to ensure Pana looks correct.
        // We don't do this for the void-engine because that
        // means Pana sets all components to empty on startup, which is
        // their responsibility.
        slotStateChanged( Engine::Empty );
    }

    m_engine = loadEngine( PanaConfig::soundSystem() );

    const QString engineName = PluginManager::getService( m_engine )->property( "X-KDE-Pana-name" ).toString();

    if( !PanaConfig::soundSystem().isEmpty() && engineName != PanaConfig::soundSystem() ) {
        //PanaConfig::soundSystem() is empty on the first-ever-run

        Pana::StatusBar::instance()->longMessage( i18n(
                "Sorry, the '%1' could not be loaded, instead we have loaded the '%2'." )
                        .arg( PanaConfig::soundSystem() )
                        .arg( engineName ),
                KDE::StatusBar::Sorry );

        PanaConfig::setSoundSystem( engineName );
    }

    // Important: Make sure soundSystem is not empty
    if( PanaConfig::soundSystem().isEmpty() )
        PanaConfig::setSoundSystem( engineName );
...
EngineController::loadEngine( const QString &engineName )
{
    ...
    QString query = "[X-KDE-Pana-plugintype] == 'engine' and [X-KDE-Pana-name] != '%1'";
    ...
    #define rank( x ) (x)->property( "X-KDE-Pana-rank" ).toInt()
    ...
    query = "[X-KDE-Pana-plugintype] == 'engine' and [X-KDE-Pana-name] == '%1'";
    offers = PluginManager::query( query.arg( engineName ) ) + offers;

    foreachType( KTrader::OfferList, offers ) {
        Pana::Plugin *plugin = PluginManager::createFromService( *it );

        if( plugin ) {
            QObject *bar = Pana::StatusBar::instance();
            ...
            connect( engine, SIGNAL(showConfigDialog( const QCString& )),
                       kapp,   SLOT(slotConfigPana( const QCString& )) );
    ...
    KMessageBox::error( 0, i18n(
            "<p>Pana could not find any sound-engine plugins. "
            "Pana is now updating the KDE configuration database. Please wait a couple of minutes, then restart Pana.</p>"
            "<p>If this does not help, "
            "it is likely that Pana is installed under the wrong prefix, please fix your installation using:<pre>"
            "$ cd /path/to/pana/source-code/<br>"
            "$ su -c \"make uninstall\"<br>"
            "$ ./configure --prefix=`kde-config --prefix` && su -c \"make install\"<br>"
            "$ kbuildsycoca<br>"
            "$ pana</pre>"
            "More information can be found in the README file. For further assistance join us at #pana on irc.freenode.net.</p>" ) );
...
bool EngineController::canDecode( const KURL &url ) //static
{
   //NOTE this function must be thread-safe
    ...
    const QString ext = Pana::extension( fileName );
    ...
    // into the playlist
    // TODO remove for Pana 1.3 and above silly checks, instead check for http type servers
    ...
    if( engine() != EngineController::instance()->m_voidEngine )
    {
        ...
        if ( !valid && ext.lower() == "mp3"){
            ...
            QApplication::postEvent( Pana::StatusBar::instance(), e );
	}
...
void EngineController::unplayableNotification() {
    ...
    if( !installDistroCodec(PanaConfig::soundSystem()))
        Pana::StatusBar::instance()->longMessageThreadSafe(
                  i18n( "<p>The %1 claims it <b>cannot</b> play MP3 files."
                        ...
                        "<p>You may find useful information in the <i>FAQ</i> section of the <i>Pana HandBook</i>." )
                    .arg( PanaConfig::soundSystem() ), KDE::StatusBar::Error );
}

bool EngineController::installDistroCodec( const QString& engine /*Filetype type*/)
{
    KService::Ptr service = KTrader::self()->query( "Pana/CodecInstall"
        , QString("[X-KDE-Pana-codec] == 'mp3' and [X-KDE-Pana-engine] == '%1'").arg(engine) ).first();
    if( service )
    {
        ...
        if( !installScript.isNull() ) //just a sanity check
        {
            ...
            , i18n("Pana currently cannot play MP3 files.")
            , i18n( "No MP3 Support" )
            , installButton
            , KStdGuiItem::no()
...
void EngineController::restoreSession()
{
    ...
    if( !PanaConfig::resumeTrack().isEmpty() )
    {
        const KURL url = PanaConfig::resumeTrack();

        play( MetaBundle( url ), PanaConfig::resumeTime() );
    }
}
...
void EngineController::play( const MetaBundle &bundle, uint offset )
{
    ...
    //let Pana know that the previous track is no longer playing
    if ( m_timer->isActive() )
    ...
    else
    {
        if( url.protocol() == "cdda" )
            Pana::StatusBar::instance()->shortMessage( i18n("Starting CD Audio track...") );
        else
            Pana::StatusBar::instance()->shortMessage( i18n("Connecting to stream source...") );
    ...
    if( m_engine->load( url, url.protocol() == "http" || url.protocol() == "rtsp" ) )
    {
        ...
        if( m_engine->play( offset ) )
        {
            ...
            m_xFadeThisTrack = !m_engine->isStream() && !(url.protocol() == "cdda") &&
                    m_bundle.length()*1000 - offset - PanaConfig::crossfadeLength()*2 > 0;
    ...
    some_kind_of_failure:
        ...
        if ( ( ( Pana::repeatPlaylist() && static_cast<int>(m_playFailureCount) < totalTracks )
            || ( Pana::repeatNone() && currentTrack != totalTracks - 1 )
            || ( Pana::repeatAlbum() && m_playFailureCount < Playlist::instance()->repeatAlbumTrackCount() ) )
            && PanaConfig::soundSystem() != "nmm-engine"
...
int EngineController::setVolume( int percent ) //SLOT
{
    ...
    if( (uint)percent != m_engine->volume() )
    {
        ...
        PanaConfig::setMasterVolume( percent );
...
void EngineController::slotMainTimer() //SLOT
{
    ...
    // Crossfading
    if ( m_engine->state() == Engine::Playing &&
         PanaConfig::crossfade() && m_xFadeThisTrack &&
         m_engine->hasPluginProperty( "HasCrossfade" ) &&
         Playlist::instance()->stopAfterMode() != Playlist::StopAfterCurrent &&
         ( (uint) PanaConfig::crossfadeType() == 0 ||    //Always or...
           (uint) PanaConfig::crossfadeType() == 1 ) &&  //...automatic track change only
         Playlist::instance()->isTrackAfter() &&
         m_bundle.length()*1000 - position < (uint) PanaConfig::crossfadeLength() )
    {
        ...
        trackFinished();
              ...
              PanaConfig::fadeout() &&
              Playlist::instance()->stopAfterMode() == Playlist::StopAfterCurrent &&
              m_bundle.length()*1000 - position < (uint) PanaConfig::fadeoutLength() )
    {
       m_engine->stop();
    }
}
...
{
    if ( PanaConfig::trackDelayLength() > 0 )
    {
        //FIXME not perfect
        if ( !m_isTiming )
        {
            QTimer::singleShot( PanaConfig::trackDelayLength(), this, SLOT(trackFinished()) );
            m_isTiming = true;
        }
