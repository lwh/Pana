...
//
//   (private slot) slotMoodbarPrefs(): Called when the Pana config changes.
...
{
    if( m_moodbarBroken  ||  !PanaConfig::showMoodbar() )
      return false;
...
MoodServer::slotNewJob( void )
{
  ...
  m_currentProcess = new Pana::Process( this );
...
MoodServer::setMoodbarBroken( void )
{
    ...
    Pana::StatusBar::instance()->longMessage( i18n(
        "The Pana moodbar analyzer program seems to be broken. "
        ...
        WEBPAGE "'>" WEBPAGE "</a>. "
        "When the problem is fixed, please restart Pana."),
        KDE::StatusBar::Error );
...
{
    if( m_state != Loaded  ||  !PanaConfig::showMoodbar() )  // Naughty caller!
      return QPixmap();
...
// (PanaConfig::AlterMood()) here, as well as calculating the
...
{
  return moodFilename( url, PanaConfig::moodsWithMusic() );
}
...
Moodbar::moodFilename( const KURL &url, bool withMusic )
{
    ...
    if( withMusic )
    ...
    else
      {
        ...
        path = ::locateLocal( "data", "pana/moods/" + path );
      }
