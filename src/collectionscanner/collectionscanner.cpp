...
CollectionScanner::CollectionScanner( const QStringList& folders,
        ...
        , m_restart( restart )
        , m_logfile( Pana::saveLocation( QString::null ) + "collection_scan.log"  )
        , m_pause( false )
{
    ...
    kapp->setName( QString( "panacollectionscanner" ).ascii() );
    if( !restart )
...
CollectionScanner::doJob() //SLOT
{
    ...
    if( m_restart ) {
        ...
        QFile folderFile( Pana::saveLocation( QString::null ) + "collection_scan.files"   );
...
{
    static DCOPRef dcopRef( "pana", "collection" );
...
CollectionScanner::scanFiles( const QStringList& entries )
{
    ...
    DCOPRef dcopRef( "pana", "collection" );
    ...
    foreachType( QStringList, entries ) {
        ...
        if( path == entries.last() || dir != directory( *itTemp ) )
        {
            ...
            foreachType( QStringList, images ) {
                // Serialize CoverBundle list with PANA_MAGIC as separator
                QString string;

                for( QValueList<CoverBundle>::ConstIterator it2 = covers.begin(); it2 != covers.end(); ++it2 )
                    string += (*it2).first + "PANA_MAGIC" + (*it2).second + "PANA_MAGIC";
