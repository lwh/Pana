...
#include "pana.h"
#include "panaconfig.h"   //used in init()
    ...
    , m_noDeviceManager( false )
{
    ...
    if ( !Pana::config( "Collection" )->readBoolEntry( "DynamicCollection", true ) )
    {
        debug() << "Dynamic Collection deactivated in panarc, not loading plugins, not connecting signals" << endl;
        return;
    }
...
{
    DEBUG_BLOCK
    KTrader::OfferList plugins = PluginManager::query( "[X-KDE-Pana-plugintype] == 'device'" );
    ...
    {
        QStringList folders = PanaConfig::collectionFolders();
        if( !folders.isEmpty() )
...
MountPointManager::collectionFolders( )
{
    //TODO max: cache data
    QStringList result;
    KConfig* const folders = Pana::config( "Collection Folders" );
...
MountPointManager::setCollectionFolders( const QStringList &folders )
{
    ...
    KConfig* const folderConf = Pana::config( "Collection Folders" );
    FolderMap folderMap;
    foreach( folders )
    {
...
MountPointManager::handleMissingMediaManager()
{
    ...
    //Pana::StatusBar::instance()->longMessage( i18n( "BlaBla" ), KDE::StatusBar::Warning );
}
