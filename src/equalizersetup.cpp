...
EqualizerSetup::EqualizerSetup()
        : KDialogBase( Pana::mainWindow(), 0, false, 0, 0, Ok, false )
{
    using Pana::Slider;
    ...
    presetAdd->setIconSet( SmallIconSet( Pana::icon( "add_playlist" ) ) );
    ...
    presetConf->setIconSet( SmallIconSet( Pana::icon( "configure" ) ) );
    ...
    m_groupBoxSliders->setChecked( PanaConfig::equalizerEnabled() );
    ...
    // Fill the combobox
    updatePresets( PanaConfig::equalizerPreset() );

    // make sure to restore the current preamp value
    m_slider_preamp->setValue( PanaConfig::equalizerPreamp() );

    // Init sliders
    presetChanged( PanaConfig::equalizerPreset() );
}
...
EqualizerSetup::presetsCache() const
{
    ...
    return Pana::saveLocation() + "equalizerpresets_save.xml";
}
...
EqualizerSetup::loadPresets()
{
    ...
    if ( !file.exists() )
        file.setName( locate( "data", "pana/data/equalizer_presets.xml" ) );
...
{
    PanaConfig::setEqualizerPreamp( m_slider_preamp->value() );
    PanaConfig::setEqualizerPreset( m_presetCombo->currentText() );
    PanaConfig::setEqualizerGains ( m_presets[ m_presetCombo->currentText() ] );
