        ...
        , m_xmlSource( 0 )
{
    ...
    foreachType( KURL::List, urls ) {
        const KURL url = Pana::detachedKURL(Pana::mostLocalURL( *it ));
        ...
        else if( ContextBrowser::hasContextProtocol( url ) )
        {
            ...
            m_URLs += ContextBrowser::expandURL(Pana::detachedKURL(url));
        }
...
UrlLoader::loadXml( const KURL &url )
{
    ...
    if( !loader.lastError().isEmpty() )
    {
        Pana::StatusBar::instance()->longMessageThreadSafe( i18n(
                ...
                "The XML in the playlist was invalid. Please report this as a bug to the Pana "
...
{
    if( version != Pana::xmlVersion() )
    {
        Pana::StatusBar::instance()->longMessageThreadSafe( i18n(
            "Your last playlist was saved with a different version of Pana than this one, "
        ...
        : m_path( path )
{
    QFile file( path );
    if( !file.open( IO_ReadOnly ) ) {
        m_error = i18n( "Pana could not open the file." );
        return;
    }
    ...
    case XML:
        m_error = i18n( "This component of Pana cannot translate XML playlists." );
        return;
    ...
    default:
        m_error = i18n( "Pana does not support this playlist format." );
        return;
    }
