...
PANA_EXPORT_PLUGIN( IpodMediaDevice )
    ...
    , m_customAction( 0 )
{
    ...
    KActionMenu *am = new KActionMenu( i18n( "iPod" ), Pana::icon( "device" ), ac );
    m_customAction = am;
...
IpodMediaDevice::rmbPressed( QListViewItem* qitem, const QPoint& point, int )
{
    ...
    if ( item )
    {
        if( item->type() == MediaItem::PLAYLISTSROOT )
        {
            menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )),
                    i18n("Create Playlist..."), CREATE_PLAYLIST );
        }
        else
        {
            menu.insertItem( SmallIconSet( Pana::icon( "playlist" ) ), i18n( "&Load" ), LOAD );
            menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), APPEND );
            menu.insertItem( SmallIconSet( Pana::icon( "fastforward" ) ), i18n( "&Queue Tracks" ), QUEUE );
        }
        menu.insertSeparator();

        menu.insertItem( SmallIconSet( Pana::icon( "collection" ) ), i18n( "&Copy Files to Collection..." ), COPY_TO_COLLECTION );
        switch( item->type() )
        {
        case MediaItem::ARTIST:
            menu.insertItem( SmallIconSet( Pana::icon( "cdrom_unmount" ) ), i18n( "Burn All Tracks by This Artist" ), BURN_ARTIST );
            menu.setItemEnabled( BURN_ARTIST, K3bExporter::isAvailable() );
            break;

        case MediaItem::ALBUM:
            menu.insertItem( SmallIconSet( Pana::icon( "cdrom_unmount" ) ), i18n( "Burn This Album" ), BURN_ALBUM );
            menu.setItemEnabled( BURN_ALBUM, K3bExporter::isAvailable() );
            break;

        default:
            menu.insertItem( SmallIconSet( Pana::icon( "cdrom_unmount" ) ), i18n( "Burn to CD as Data" ), BURN_DATACD );
            menu.setItemEnabled( BURN_DATACD, K3bExporter::isAvailable() );
            menu.insertItem( SmallIconSet( Pana::icon( "cdaudio_unmount" ) ), i18n( "Burn to CD as Audio" ), BURN_AUDIOCD );
                 ...
                 || item->type() == MediaItem::PODCASTCHANNEL) )
        {
            ...
            menu.insertItem( SmallIconSet( Pana::icon( "podcast" ) ), i18n( "Subscribe to This Podcast" ), SUBSCRIBE );
        ...
        case MediaItem::PODCASTITEM:
            if(m_playlistItem)
            {
                menu.insertItem( SmallIconSet( Pana::icon( "playlist" ) ), i18n( "Make Media Device Playlist" ), MAKE_PLAYLIST );
                ...
                {
                    playlistsMenu->insertItem( SmallIconSet( Pana::icon( "playlist" ) ), it->text(0), FIRST_PLAYLIST+i );
                    i++;
                }
                menu.insertItem( SmallIconSet( Pana::icon( "playlist" ) ), i18n("Add to Playlist"), playlistsMenu, ADD_TO_PLAYLIST );
            ...
            {
                menu.insertItem( SmallIconSet( Pana::icon( "edit" ) ),
        ...
        case MediaItem::ORPHANED:
        case MediaItem::ORPHANEDROOT:
            menu.insertItem( SmallIconSet( Pana::icon( "edit" ) ), i18n( "Add to Database" ), ADD );
        ...
        {
            menu.insertItem( SmallIconSet( Pana::icon( "remove_from_playlist" ) ),
        ...
        {
            menu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n( "Delete Podcasts Already Played" ), DELETE_PLAYED );
            menu.setItemEnabled( DELETE_PLAYED, !locked );
        }
        menu.insertItem( SmallIconSet( Pana::icon( "remove" ) ),
            ...
            case ADD:
                {
                    ...
                    if( dupes > 0 )
                        Pana::StatusBar::instance()->shortMessage( i18n(
...
IpodMediaDevice::addConfigElements( QWidget *parent )
{
    ...
    m_syncStatsCheck->setText( i18n( "&Synchronize with Pana statistics" ) );
    QToolTip::add( m_syncStatsCheck, i18n( "Synchronize with Pana statistics and submit tracks played to last.fm" ) );
