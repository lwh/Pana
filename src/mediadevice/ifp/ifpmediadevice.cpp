 ...
 * copyright (c) 2004 Thomas Loeber <ifp@loeber1.de>
 *                                                                         *
...
PANA_EXPORT_PLUGIN( IfpMediaDevice )
...
IfpMediaDevice::openDevice( bool /*silent*/ )
{
    ...
    if( m_dh == NULL )
    {
        ...
        Pana::StatusBar::instance()->shortLongMessage( genericError,
...
{
//     KConfig *config = Pana::config( "MediaDevice" );
...
IfpMediaDevice::rmbPressed( QListViewItem* qitem, const QPoint& point, int )
{
    ...
    if ( item )
    {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "collection" ) ), i18n( "Download" ), DOWNLOAD );
...
{
    QString result = Pana::asciiPath( component );
