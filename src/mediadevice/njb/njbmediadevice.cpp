...
PANA_EXPORT_PLUGIN( NjbMediaDevice )
...
NjbMediaDevice::openDevice(bool)
{
    ...
    {
        Pana::StatusBar::instance()->shortLongMessage( genericError, i18n("A suitable Nomad device could not be found"), KDE::StatusBar::Error );
        debug() << ": no NJBs found\n";
    ...
    if( NJB_Open( m_njb ) == -1 )
    {
        Pana::StatusBar::instance()->shortLongMessage( genericError, i18n("Nomad device could not be opened"), KDE::StatusBar::Error );
...
NjbMediaDevice::deleteTrack(NjbMediaItem *trackItem)
{
    ...
    {
        debug() << ": NJB_Delete_Track failed" << endl;
        Pana::StatusBar::instance()->shortLongMessage( i18n( "Deleting failed" ), i18n( "Deleting track(s) failed." ), KDE::StatusBar::Error );
        return -1;
    }
...
NjbMediaDevice::newPlaylist(const QString& name, MediaItem* parent, QPtrList< MediaItem > items)
{
    ...
    if(status == NJB_SUCCESS)
    {
        //NjbMediaItem *newNjbPlayList = new NjbMediaItem(listPanaPlayLists);
    ...
    //TODO:Crear un conversor de Playlists a las listas de Pana
    return 0;
}
...
NjbMediaDevice::rmbPressed(QListViewItem* qitem, const QPoint& point, int )
{
    ...
    if ( item )
    {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "collection" ) ), i18n("Download file"), DOWNLOAD );
        menu.insertItem( SmallIconSet( Pana::icon( "collection" ) ), i18n("Download to collection"), DOWNLOAD_TO_COLLECTION );
        menu.insertSeparator();
        //menu.insertItem( SmallIconSet( Pana::icon( "edit" ) ), i18n( "Rename" ), RENAME );
        menu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n( "Delete from device" ), DELETE );
