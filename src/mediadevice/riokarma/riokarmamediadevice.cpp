...
PANA_EXPORT_PLUGIN( RioKarmaMediaDevice )
...
RioKarmaMediaDevice::openDevice( bool silent )
{
    ...
    if( m_rio < 0 )
    {
        ...
        Pana::StatusBar::instance()->shortLongMessage( genericError, i18n( "Rio Karma could not be opened" ), KDE::StatusBar::Error );
        setDisconnected();
        return false;
    }
...
RioKarmaMediaDevice::readKarmaMusic()
{
    ...
    if( ret == 0 )
    {
        ...
        Pana::StatusBar::instance()->shortLongMessage( genericError, i18n( "Could not read Rio Karma tracks" ), KDE::StatusBar::Error );
        setDisconnected();
        hideProgress();
        return -1;
    }
