...
#ifndef PANA_DAAPCLIENT_CPP
#define PANA_DAAPCLIENT_CPP
...
#endif
    QStringList sl = PanaConfig::manuallyAddedServers();
    foreach( sl )
    {
...
DaapClient::rmbPressed( QListViewItem* qitem, const QPoint& point, int )
{
    ...
    switch( item->type() )
    {
        ...
        default:
            ...
            menu.insertItem( SmallIconSet( Pana::icon( "playlist" ) ), i18n( "&Copy Files to Collection..." ), DOWNLOAD );

            // albums and artists don't have bundles, so they crash... :(
            if( item->bundle() )
            {
                menu.insertItem( SmallIconSet( Pana::icon( "info" ) ), i18n( "Track &Information..." ), INFO );
            }
            break;
    }
    ...
    switch( id )
    {
        ...
        case REMOVE:
            if( sitem )
            {
                QStringList mas = PanaConfig::manuallyAddedServers();
                mas.remove( sitem->key() );
                PanaConfig::setManuallyAddedServers( mas );
                delete sitem;
            }
            break;
        case DOWNLOAD:
...
DaapClient::customClicked()
{
    class AddHostDialog : public KDialogBase
    {
        ...
        public:
                ...
                : KDialogBase( parent, "DaapAddHostDialog", true, i18n( "Add Computer" ) , Ok|Cancel)
            {
                ...
                m_base->m_downloadPixmap->setPixmap( QPixmap( KGlobal::iconLoader()->iconPath( Pana::icon( "download" ), -KIcon::SizeEnormous ) ) );
    ...
    if( dialog.exec() == QDialog::Accepted ) {
        ...
        if( ip == "0" )
            Pana::StatusBar::instance()->shortMessage( i18n("Could not resolve %1.").arg( dialog.m_base->m_hostName->text() ) );
        else
        {
            QString key = ServerItem::key( dialog.m_base->m_hostName->text(), dialog.m_base->m_portInput->value() );
            if( !PanaConfig::manuallyAddedServers().contains( key ) )
            {
                QStringList mas = PanaConfig::manuallyAddedServers();
                mas.append( key );
                PanaConfig::setManuallyAddedServers( mas );
...
ServerItem::httpError( const QString& errorString )
{
    stopAnimation();
    resetTitle();
    Pana::StatusBar::instance()->longMessage( i18n( "The following error occurred while trying to connect to the remote server:<br>%1").arg( errorString ) );
...
{
 //   Pana::StatusBar::instance()->longMessageThreadSafe( i18n( "An error occured while downloading from remote music server." ), Pana::StatusBar::Error );
    DEBUG_BLOCK
