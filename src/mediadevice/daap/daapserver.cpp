  ...
  , m_service( 0 )
{
    ...
    *m_server << "pana_daapserver.rb";
    *m_server << locate( "data", "pana14/ruby_lib/" );
    *m_server << locate( "lib", "pana14/ruby_lib/" );
    *m_server << locate( "data", "pana14/scripts/ruby_debug/debug.rb" );
    if( !m_server->start( KProcIO::NotifyOnExit, true ) ) {
        error() << "Failed to start pana_daapserver.rb" << endl;
        return;
    }
    ...
    while( m_server->readln( line ) != -1 )
    {
        ...
        else if( line.startsWith( serverStartPrefix ) )
        {
            ...
            debug() << "Server starting on port " << line << '.' << endl;
                ...
                KUser current;
                if( !m_service )
                    m_service = new DNSSD::PublicService( i18n("%1's pana Share").arg( current.fullName() ), "_daap._tcp", line.toInt() );
