...
/*
        bundle->setUrl( Pana::QStringx("http://%1:3689/databases/%2/items/%3.%4?%5").args(
            QStringList() << m_host
                        << m_databaseId
    ...
    , m_proxy( new Pana::ProcIO() )
{
    ...
    *m_proxy << "pana_proxy.rb";
    *m_proxy << "--daap";
    *m_proxy << QString::number( port );
    *m_proxy << realStream.url();
    *m_proxy << PanaConfig::soundSystem();
    *m_proxy << hash;
    *m_proxy << QString::number( revisionId );
    *m_proxy << Pana::proxyForUrl( realStream.url() );
    ...
    debug() << "started pana_proxy.rb --daap " << QString::number( port ) << ' ' << realStream.url() << ' ' << PanaConfig::soundSystem() << ' ' << hash << ' ' << revisionId << endl;
