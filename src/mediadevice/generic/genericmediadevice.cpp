...
PANA_EXPORT_PLUGIN( GenericMediaDevice )
...
GenericMediaDevice::openDevice( bool /*silent*/ )
{
    ...
    {
        Pana::StatusBar::instance()->longMessage( i18n( "Devices handled by this plugin must be mounted first.\n"
...
GenericMediaDevice::rmbPressed( QListViewItem* qitem, const QPoint& point, int )
{
    ...
    if ( item )
    {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "1downarrow" ) ), i18n( "&Append to Playlist" ), APPEND );
        menu.insertItem( SmallIconSet( Pana::icon( "fastforward" ) ), i18n( "&Queue Tracks" ), QUEUE );
        menu.insertSeparator();
        menu.insertItem( SmallIconSet( Pana::icon( "collection" ) ), i18n( "&Copy Files to Collection..." ), DOWNLOAD );
        ...
        menu.insertSeparator();
        menu.insertItem( SmallIconSet( Pana::icon( "folder" ) ), i18n( "Add Directory" ), DIRECTORY );
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "Transfer Queue to Here..." ), TRANSFER_HERE );
...
{
    QString result = Pana::cleanPath( component );

    if( m_asciiTextOnly )
        result = Pana::asciiPath( result );

    result.simplifyWhiteSpace();
    if( m_spacesToUnderscores )
        result.replace( QRegExp( "\\s" ), "_" );
    if( m_actuallyVfat || m_vfatTextOnly )
        result = Pana::vfatPath( result );
