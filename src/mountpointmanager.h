 ...
 */
#ifndef PANA_MOUNTPOINTMANAGER_H
#define PANA_MOUNTPOINTMANAGER_H
...
class LIBPANA_EXPORT DeviceHandlerFactory : public Pana::Plugin
{
public:
...
public:
    ...
    /**
     * returns an absolute path which is guaranteed to be playable by pana's current engine. (based on an
     ...
     * is not necessarily playable! (based on an idea by andrewt512: allows better handling of files stored in remote  * collections. this method would return a "pretty" URL which might not be playable by pana's engines.
