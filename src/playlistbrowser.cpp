...
#include "pana.h"            //actionCollection()
...
{
    return Pana::extension( fileName );
}
...

{
    ...
    //<Toolbar>
    addMenuButton  = new KActionMenu( i18n("Add"), Pana::icon( "add_playlist" ), m_ac );
    ...
    removeButton   = new KAction( i18n("Delete"), Pana::icon( "remove" ), 0, this, SLOT( removeSelectedItems() ), m_ac );
    ...
    int sort = Pana::config( "PlaylistBrowser" )->readNumEntry( "Sorting", Qt::Ascending );
    m_listview->setSorting( 0, sort == Qt::Ascending ? true : false );

    m_podcastTimerInterval = Pana::config( "PlaylistBrowser" )->readNumEntry( "Podcast Interval", 14400000 );
...
#define config Pana::config( "PlaylistBrowser" )
...
#undef config
    ...
    if( !PanaConfig::scrobblerUsername().isEmpty() )
    {
        const bool subscriber = Pana::config( "Scrobbler" )->readBoolEntry( "Subscriber", false );
    ...
    QValueList<int> stateList = Pana::config( "PlaylistBrowser" )->readIntListEntry( "Item State" );
    ...
    m_infoPane->setStoredHeight( Pana::config( "PlaylistBrowser" )->readNumEntry( "InfoPane Height", 200 ) );
}
...
PlaylistBrowser::~PlaylistBrowser()
{
    ...
    if( m_polished )
    {
        ...
        Pana::config( "PlaylistBrowser" )->writeEntry( "Sorting", m_listview->sortOrder() );
        Pana::config( "PlaylistBrowser" )->writeEntry( "Podcast Interval", m_podcastTimerInterval );
        Pana::config( "PlaylistBrowser" )->writeEntry( "Podcast Folder Open", m_podcastCategory->isOpen() );
        Pana::config( "PlaylistBrowser" )->writeEntry( "InfoPane Height", m_infoPane->getHeight() );
    }
}
...
PlaylistBrowser::markDynamicEntries()
{
    if( Pana::dynamicMode() )
    {
        QStringList playlists = Pana::dynamicMode()->items();

        for( uint i=0; i < playlists.count(); i++ )
        {
            PlaylistBrowserEntry *item = dynamic_cast<PlaylistBrowserEntry*>( Pana::findItemByPath( m_listview, playlists[i]  ) );
...
{
    return Pana::saveLocation() + "streambrowser_save.xml";
}
...
{
    QFile file( locate( "data","pana/data/Cool-Streams.xml" ) );
...
void PlaylistBrowser::saveStreams()
{
    ...
    streamB.setAttribute( "product", "Pana" );
...
{
    QFile file( Pana::saveLocation() + "lastfmbrowser_save.xml" );
    ...
    QString user = PanaConfig::scrobblerUsername();
...
void PlaylistBrowser::saveLastFm()
{
    ...
    lastfmB.setAttribute( "product", "Pana" );
...
{
    return Pana::saveLocation() + "smartplaylistbrowser_save.xml";
}
...
void PlaylistBrowser::loadDefaultSmartPlaylists()
{
    ...
    item->setPixmap( 0, SmallIcon( Pana::icon( "collection" ) ) );
    item->setKept( false );
    /********** Favorite Tracks **************/
    qb.initSQLDrag();
    qb.sortByFavorite();
...
void PlaylistBrowser::saveSmartPlaylists( PlaylistCategory *smartCategory )
{
    ...
    smartB.setAttribute( "product", "Pana" );
...
{
    return Pana::saveLocation() + "dynamicbrowser_save.xml";
}
...
{
    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Random Remove Played",  m_randomDynamic->cycleTracks() );
    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Random Upcoming Count", m_randomDynamic->upcomingCount() );
    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Random Previous Count", m_randomDynamic->previousCount() );

    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Suggest Remove Played",  m_suggestedDynamic->cycleTracks() );
    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Suggest Upcoming Count", m_suggestedDynamic->upcomingCount() );
    Pana::config( "PlaylistBrowser" )->writeEntry( "Dynamic Suggest Previous Count", m_suggestedDynamic->previousCount() );
    ...
    dynamicB.setAttribute( "product", "Pana" );
...
QString PlaylistBrowser::podcastBrowserCache() const
{
    ...
    return Pana::saveLocation() + "podcastbrowser_save.xml";
}
...
DEBUG_BLOCK
    ...
    p->setOpen( Pana::config( "PlaylistBrowser" )->readBoolEntry( "Podcast Folder Open", true ) );
...
{
    //returns the playlists stats cache file
    return Pana::saveLocation() + "playlistbrowser_save.xml";
}
...
void PlaylistBrowser::savePlaylists()
{
    ...
    playlistsB.setAttribute( "product", "Pana" );
...
QString PlaylistDialog::getSaveFileName( const QString &suggestion, bool proposeOverwriting ) //static
{
    ...
    {
        QString path = Pana::saveLocation("playlists/") + "%1" + ".m3u";
...
PlaylistDialog::PlaylistDialog()
    : KDialogBase( PlaylistWindow::self(), "saveplaylist", true /*modal*/,
                   ...
                   KGuiItem( i18n( "Save to location..." ), SmallIconSet( Pana::icon( "files" ) ) ) )
    , customChosen( false )
{
...
void PlaylistDialog::slotOk()
{
    ...
    QString folder = Pana::saveLocation( "playlists" );
    QFileInfo info( folder );
    if ( !info.isDir() ) QFile::remove( folder );

    if( !customChosen && !edit->text().isEmpty() )
        result = Pana::saveLocation( "playlists/" ) + edit->text() + ".m3u";
