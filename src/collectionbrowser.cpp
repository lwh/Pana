...
namespace Pana { extern KConfig *config( const QString& ); }
    ...
    , m_returnPressed( false )
{
    ...
    m_configureAction = new KAction( i18n( "Configure Folders" ), Pana::icon( "configure" ), 0, this, SLOT( setupDirs() ), ac, "Configure" );
    ...
    m_ipodViewAction = new KRadioAction( i18n( "iPod View" ), Pana::icon("device"), 0, m_view, SLOT( setIpodMode() ), ac, "iPod View" );
...
CollectionView::~CollectionView() {
    ...
    KConfig* const config = Pana::config( "Collection Browser" );
...
{
    Pana::actionCollection()->action("update_collection")->setEnabled( false );
}
...
CollectionView::scanDone( bool changed ) //SLOT
{
    ...
    Pana::actionCollection()->action("update_collection")->setEnabled( true );
}
...
CollectionView::rmbPressed( QListViewItem* item, const QPoint& point, int ) //SLOT
{
    ...
    if ( item ) {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), selection.count() == 1 ? i18n( "&Queue Track" )
            : i18n( "&Queue Tracks" ), QUEUE );

        if( selection.count() > 1 || item->isExpandable() )
            menu.insertItem( SmallIconSet( Pana::icon( "save" ) ), i18n( "&Save as Playlist..." ), SAVE );
        ...
        if( cat == IdArtist )
        {
            menu.insertItem( SmallIconSet( Pana::icon( "burn" ) ), i18n( "&Burn All Tracks by This Artist" ), BURN_ARTIST );
            menu.setItemEnabled( BURN_ARTIST, K3bExporter::isAvailable() && siblingSelection.count() == 1 );
        }
        else if( cat == IdComposer )
        {
            menu.insertItem( SmallIconSet( Pana::icon( "burn" ) ), i18n( "&Burn All Tracks by This Composer" ), BURN_COMPOSER );
            menu.setItemEnabled( BURN_COMPOSER, K3bExporter::isAvailable() && siblingSelection.count() == 1 );
        }
        else if( (cat == IdAlbum || cat == IdVisYearAlbum) )
        {
            menu.insertItem( SmallIconSet( Pana::icon( "burn" ) ), i18n( "&Burn This Album" ), BURN_ALBUM );
        ...
        {
            menu.insertItem( SmallIconSet( Pana::icon( "burn" ) ), i18n( "B&urn to CD" ), BURN_CD );
        ...
        fileMenu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n( "&Delete File..." , "&Delete %n Files..." , selection.count() ) , DELETE );
        menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "Manage &Files" ), &fileMenu, FILE_MENU );

		  // cover fetch isn't multiselection capable
        if( (cat == IdAlbum || cat == IdVisYearAlbum) && siblingSelection.count() == 1 )
        {
            menu.insertItem( SmallIconSet( Pana::icon( "download" ) ), i18n( "&Fetch Cover From Last.fm" ), this, SLOT( fetchCover() ), 0, FETCH );
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "info" ) )
...
CollectionView::organizeFiles( const KURL::List &urls, const QString &caption, bool copy )  //SLOT
{
    if( m_organizingFileCancelled )
    {
        ...
        Pana::StatusBar::instance()->shortMessage( shortMsg, KDE::StatusBar::Sorry );
        return;
    }
    ...
    if( m_organizeURLs.count() )
    {
        ...
        {
            m_organizeURLs += Pana::recursiveUrlExpand( urls );
            Pana::StatusBar::instance()->incrementProgressTotalSteps( this, urls.count() );
            return;
        }
    }
    ...
    if( folders.isEmpty() )
    {
        ...
        Pana::StatusBar::instance()->longMessage( longMsg, KDE::StatusBar::Sorry );
        return;
    }
    ...
    dialog.folderCombo->setCurrentItem( PanaConfig::organizeDirectory() );
    dialog.overwriteCheck->setChecked( PanaConfig::overwriteFiles() );
    dialog.filetypeCheck->setChecked( PanaConfig::groupByFiletype() );
    dialog.initialCheck->setChecked( PanaConfig::groupArtists() );
    dialog.spaceCheck->setChecked( PanaConfig::replaceSpace() );
    dialog.coverCheck->setChecked( PanaConfig::coverIcons() );
    dialog.ignoreTheCheck->setChecked( PanaConfig::ignoreThe() );
    dialog.vfatCheck->setChecked( PanaConfig::vfatCompatible() );
    dialog.asciiCheck->setChecked( PanaConfig::asciiOnly() );
    dialog.customschemeCheck->setChecked( PanaConfig::useCustomScheme() );
    dialog.formatEdit->setText( PanaConfig::customScheme() );
    dialog.regexpEdit->setText( PanaConfig::replacementRegexp() );
    dialog.replaceEdit->setText( PanaConfig::replacementString() );
    ...
    KURL::List previewURLs = Pana::recursiveUrlExpand( urls.first(), 1 );
    ...
    {
        PanaConfig::setOrganizeDirectory( dialog.folderCombo->currentItem() );
        PanaConfig::setOverwriteFiles( dialog.overwriteCheck->isChecked() );
        PanaConfig::setGroupByFiletype( dialog.filetypeCheck->isChecked() );
        PanaConfig::setGroupArtists( dialog.initialCheck->isChecked() );
        PanaConfig::setIgnoreThe( dialog.ignoreTheCheck->isChecked() );
        PanaConfig::setReplaceSpace( dialog.spaceCheck->isChecked() );
        PanaConfig::setCoverIcons( dialog.coverCheck->isChecked() );
        PanaConfig::setVfatCompatible( dialog.vfatCheck->isChecked() );
        PanaConfig::setAsciiOnly( dialog.asciiCheck->isChecked() );
        PanaConfig::setUseCustomScheme( dialog.customschemeCheck->isChecked() );
        PanaConfig::setCustomScheme( dialog.formatEdit->text() );
        PanaConfig::setReplacementRegexp( dialog.regexpEdit->text() );
        PanaConfig::setReplacementString( dialog.replaceEdit->text() );
        KURL::List skipped;

        m_organizeURLs = Pana::recursiveUrlExpand( urls );
        m_organizeCopyMode = copy;
        CollectionDB::instance()->createTables( true ); // create temp tables
        Pana::StatusBar::instance()->newProgressOperation( this )
        ...
        while( !m_organizeURLs.empty() && !m_organizingFileCancelled )
        {
            ...
            Pana::StatusBar::instance()->incrementProgress( this );
        ...
        if( !m_organizingFileCancelled && skipped.count() > 0 )
        {
            ...
            Pana::StatusBar::instance()->shortLongMessage( shortMsg, longMsg, KDE::StatusBar::Sorry );
        }
        else if ( m_organizingFileCancelled )
        {
            Pana::StatusBar::instance()->shortMessage( i18n( "Aborting jobs..." ) );
            m_organizingFileCancelled = false;
        }

        m_dirty = true;
        QTimer::singleShot( 0, CollectionView::instance(), SLOT( renderView() ) );
        Pana::StatusBar::instance()->endProgressOperation( this );
    }
}
    ...
    if( KURLDrag::decode( e, list ) )
    {
                ...
                ++it )
        {
            if( (*it).isLocalFile() && QFileInfo( (*it).path() ).isDir() )
                expandedList += Pana::recursiveUrlExpand( *it );
            else
                expandedList += *it;
        }
...
{
    return SmallIcon( Pana::icon( "fastforward" ) );
}

QPixmap
CollectionView::ipodDecrementIcon ( void )
{
    return SmallIcon( Pana::icon( "rewind" ) );
}
...
CollectionView::eventFilter( QObject* o, QEvent* e )
{
        ...
        && m_viewMode == modeFlatView )
    {
        ...
        popup.setItemVisible( Score, PanaConfig::useScores() );
        popup.setItemVisible( Rating, PanaConfig::useRatings() );
