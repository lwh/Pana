...
//    delete m_podcastBundle; why does this crash Pana? apparently m_podcastBundle isn't always initialized.
    m_podcastBundle = 0;
...
QString MetaBundle::prettyText( int column ) const
{
    ...
    switch( column )
    {
        ...
        case LastPlayed: text = Pana::verboseTimeSince( lastPlay() );                              break;
...
MetaBundle::setExtendedTag( TagLib::File *file, int tag, const QString value )
{
    ...
    else if ( m_type == ogg )
    {
        switch( tag )
        {
            ...
            case ( albumArtistTag ): id = "ALBUMARTIST"; break; // non-standard: Pana
        }
...
{
    m_url = Pana::detachedKURL( m_url );
    m_localUrl = Pana::detachedKURL( m_localUrl );
    m_parent = Pana::detachedKURL( m_parent );
