...
/// @class Pana::SocketServer

Pana::SocketServer::SocketServer( const QString &socketName, QObject *parent )
...
Pana::SocketServer::~SocketServer()
{
    if( m_sockfd != -1 )
        ::close( m_sockfd );
}
...
Vis::SocketServer::SocketServer( QObject *parent )
        : Pana::SocketServer( "pana.visualization_socket", parent )
{}
...
{
    Pana::OverrideCursor waitcursor;
    ...
    char str[4096];
    FILE* vis = popen( "pana_libvisual --list", "r" );
    ...
    for( QStringList::ConstIterator it = entries.begin(); it != entries.end(); ++it )
        new Item( this, "pana_libvisual", *it, "libvisual" );
...
// Shouldn't be necessary, but it's part of a fix to make libvisual work again when running with pana binary
void
...
Vis::Selector::viewportPaintEvent( QPaintEvent *e )
{
    if( childCount() == 0 ) {
        ...
        //TODO the right message if pana_libvisual is present but libvisual isn't
        hide();
        Pana::StatusBar::instance()->longMessage( i18n(
                ...
                "</ul>"
                 "Please check these possibilities and restart Pana."
...
Vis::Selector::Item::stateChange( bool ) //SLOT
{
    ...
    case On:
       ...
       *m_proc << KStandardDirs::findExe( m_command )
        ...
        // Shouldn't be necessary, but make visualizations work again when running with pana binary
