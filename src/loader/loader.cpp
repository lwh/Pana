...
{
    //NOTE this list doesn't include argv[0] ("pana")
    QStringList args;
    for( int i = 1; i < argc; i++ )
        args += QString::fromLocal8Bit(argv[i]);

    const bool isRunning = panaIsRunning();
    ...
    if( !args.isEmpty() )
    {
        // These arguments cannot be passed to Pana, or Pana will exit
        ...
        foreach( args ) {
            ...
            foreach( longs )
                if( arg == *it ) {
                    // this argument cannot be passed to the running panaapp
                    // or KCmdLineArgs would exit the application

                    QProcess proc( QString("panaapp") );
        ...
        // there is no panaapp running, we'll start a new
        // instance and the above checks were not necessary
    }

    if ( isRunning ) {
        QStringList dcop_args;
        dcop_args << "dcop" << "pana" << "player" << "transferCliArgs" << "[";

        // We transmit our DESKTOP_STARTUP_ID, so panaapp can stop the startup animation
        dcop_args += std::getenv( "DESKTOP_STARTUP_ID" ); //will be interptreted as latin1

        // relative URLs should be interpreted correctly by panaapp
        ...
        // no panaapp is running, start one, show
...
Loader::Loader( QStringList args )
        ...
        , m_splash( 0 )
{
    // we transmit the startup_id, so panaapp can stop the startup animation
    //FIXME QCString str( ::getenv( "DESKTOP_STARTUP_ID" ) );

     if( !QApplication::isSessionRestored())
     {
        KInstance instance("pana"); // KGlobal::dirs() crashes without
        if( isSplashEnabled() )
        {
            m_splash = new KSplashScreen( QPixmap( KStandardDirs().findResource("data", "pana/images/splash_screen.jpg")));
            m_splash->show();
        }
     }

    args.prepend( "panaapp" );

    m_proc = new QProcess( args, this );
    m_proc->setCommunication( QProcess::Stdout );

    std::cout << "Pana: [Loader] Starting panaapp..\n";
    std::cout << "Pana: [Loader] Don't run gdb, valgrind, etc. against this binary! Use panaapp.\n";

    if( !m_proc->start() )
    {
        delete m_splash; // hide the splash

        QMessageBox::critical( 0, "Pana",
                "Pana could not be started!\n" //FIXME this needs to be translated
                    "This may be because the panaapp binary is not in your PATH.\n"
                    "Try locating and running panaapp from a terminal.",
                QMessageBox::Ok, 0 );
...
Loader::timerEvent( QTimerEvent* )
{
    if( m_proc->isRunning() )
    {
         if( ++m_counter == (30000 / INTERVAL) )
             ...
             std::cerr << "Pana: [Loader] Pana is taking a long time to load! Perhaps something has gone wrong?\n";
        ...
        // no reason to show messagebox, as panaapp should start drkonqi
        std::cerr << "Pana: [Loader] panaapp probably crashed!\n";

        QApplication::exit( 3 );
    }
    else
        // if we get here, then either we didn't receive STARTUP through
        // the pipe, or panaapp exited normally before the STARTUP was
...
isSplashEnabled()
{
    //determine whether splash-screen is enabled in panarc
    (void)KGlobal::config(); // the kubuntu special directory is not present without this
    QStringList dirs = KGlobal::dirs()->findAllResources( "config", "panarc" );
