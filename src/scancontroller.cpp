/***************************************************************************
 *   Copyright (C) 2003-2005 by The Pana Developers                      *
 *                                                                         *
    ...
    , m_scanCount( 0 )
{
    ...
    *m_scanner << "panacollectionscanner";
    *m_scanner << "--nocrashhandler"; // We want to be able to catch SIGSEGV
    ...
    else
    {
        ...
        *m_scanner << "-p";
        if( PanaConfig::scanRecursively() ) *m_scanner << "-r";
        *m_scanner << m_folders;
        m_scanner->start();
    }
}
...
ScanController::initIncremental()
{
    ...
    if ( !m_folders.isEmpty() )
    {
        ...
        m_hasChanged = true;
        Pana::StatusBar::instance()->shortMessage( i18n( "Updating Collection..." ) );
...
main_loop:
    ...
    if( !isAborted() ) {
        if( m_scanner->normalExit() && !m_scanner->signalled() ) {
            ...
            //that took too long, sometimes causing Pana to be killed.
...
ScanController::requestPause()
{
    ...
    DCOPRef dcopRef( "panacollectionscanner", "scanner" );
...
ScanController::startElement( const QString&, const QString& localName, const QString&, const QXmlAttributes& attrs )
{
    ...
    if( localName == "itemcount") {
        ...
        QStringList list = QStringList::split( "PANA_MAGIC", attrs.value( "list" ), true );
...
ScanController::customEvent( QCustomEvent* e )
{
    if( e->type() == RestartEventType )
    {
        ...
        QFile log( Pana::saveLocation( QString::null ) + "collection_scan.log" );
