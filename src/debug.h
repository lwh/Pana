 ...
 *
 * See top level COPYING file for licence
 */
...
namespace Debug
{
    ...
    #undef qApp
		...
		#ifndef DEBUG_PREFIX
			#define PANADEBUG_PREFIX "[PANA]"
		#else
			#define PANADEBUG_PREFIX "[PANA][" DEBUG_PREFIX "] "
		#endif
       ...
       static inline kdbgstream debug()   { mutex.lock(); QCString ind = indent(); mutex.unlock(); return kdbgstream( ind, 0, KDEBUG_INFO  ) << PANADEBUG_PREFIX; }
		static inline kdbgstream warning() { mutex.lock(); QCString ind = indent(); mutex.unlock(); return kdbgstream( ind, 0, KDEBUG_WARN ) << PANADEBUG_PREFIX << "[WARNING!] "; }
		static inline kdbgstream error()   { mutex.lock(); QCString ind = indent(); mutex.unlock(); return kdbgstream( ind, 0, KDEBUG_ERROR ) << PANADEBUG_PREFIX << "[ERROR!] "; }
		static inline kdbgstream fatal()   { mutex.lock(); QCString ind = indent(); mutex.unlock(); return kdbgstream( ind, 0, KDEBUG_FATAL ) << PANADEBUG_PREFIX; }


		typedef kdbgstream DebugStream;
		#undef PANADEBUG_PREFIX
	#endif
...
#define PANA_NOTIMPLEMENTED warning() << "NOT-IMPLEMENTED: " << __PRETTY_FUNCTION__ << endl;

/// Use this to alert other developers to stop using a function
#define PANA_DEPRECATED warning() << "DEPRECATED: " << __PRETTY_FUNCTION__ << endl;
