...
namespace Pana
{
    ...
    protected:
        ...
        contextMenuEvent( QContextMenuEvent *e ) {
            Pana::Menu::instance()->popup( e->globalPos() );
        }
        ...
        , m_lastBrowser( 0 )
{
    ...
    KStdAction::preferences( kapp, SLOT( slotConfigPana() ), ac );
    ac->action("options_configure_globals")->setIcon( Pana::icon( "configure" ) );
    ac->action(KStdAction::name(KStdAction::KeyBindings))->setIcon( Pana::icon( "configure" ) );
    ac->action(KStdAction::name(KStdAction::ConfigureToolbars))->setIcon( Pana::icon( "configure" ) );
    ac->action(KStdAction::name(KStdAction::Preferences))->setIcon( Pana::icon( "configure" ) );

    KStdAction::quit( kapp, SLOT( quit() ), ac );
    KStdAction::open( this, SLOT(slotAddLocation()), ac, "playlist_add" )->setText( i18n("&Add Media...") );
    ac->action( "playlist_add" )->setIcon( Pana::icon( "files" ) );
    KStdAction::open( this, SLOT(slotAddStream()), ac, "stream_add" )->setText( i18n("&Add Stream...") );
    ac->action( "stream_add" )->setIcon( Pana::icon( "files" ) );
    KStdAction::save( this, SLOT(savePlaylist()), ac, "playlist_save" )->setText( i18n("&Save Playlist As...") );
    ac->action( "playlist_save" )->setIcon( Pana::icon( "save" ) );

    //FIXME: after string freeze rename to "Burn Current Playlist"?
    new KAction( i18n("Burn to CD"), Pana::icon( "burn" ), 0, this, SLOT(slotBurnPlaylist()), ac, "playlist_burn" );
    actionCollection()->action("playlist_burn")->setEnabled( K3bExporter::isAvailable() );
    new KAction( i18n("Play Media..."), Pana::icon( "files" ), 0, this, SLOT(slotPlayMedia()), ac, "playlist_playmedia" );
    new KAction( i18n("Play Audio CD"), Pana::icon( "album" ), 0, this, SLOT(playAudioCD()), ac, "play_audiocd" );
    KAction *playPause = new KAction( i18n( "&Play/Pause" ), Pana::icon( "play" ), Key_Space, ec, SLOT( playPause() ), ac, "play_pause" );
    new KAction( i18n("Script Manager"), Pana::icon( "scripts" ), 0, this, SLOT(showScriptSelector()), ac, "script_manager" );
    new KAction( i18n("Queue Manager"), Pana::icon( "queue" ), 0, this, SLOT(showQueueManager()), ac, "queue_manager" );
    KAction *seekForward = new KAction( i18n( "&Seek Forward" ), Pana::icon( "fastforward" ), Key_Right, ec, SLOT( seekForward() ), ac, "seek_forward" );
    KAction *seekBackward = new KAction( i18n( "&Seek Backward" ), Pana::icon( "rewind" ), Key_Left, ec, SLOT( seekBackward() ), ac, "seek_backward" );
    new KAction( i18n("Statistics"), Pana::icon( "info" ), 0, this, SLOT(showStatistics()), ac, "statistics" );
    new KAction( i18n("Update Collection"), Pana::icon( "refresh" ), 0, CollectionDB::instance(), SLOT( scanModifiedDirs() ), actionCollection(), "update_collection" );
    ...
    KActionMenu* playLastfm = new KActionMenu( i18n( "Play las&t.fm Stream" ), Pana::icon( "audioscrobbler" ), ac, "lastfm_play" );
    ...
    KActionMenu* addLastfm = new KActionMenu( i18n( "Add las&t.fm Stream" ), Pana::icon( "audioscrobbler" ), ac, "lastfm_add" );
    ...
    new KAction( i18n( "Previous Track" ), Pana::icon( "back" ), 0, ec, SLOT( previous() ), ac, "prev" );
    new KAction( i18n( "Play" ), Pana::icon( "play" ), 0, ec, SLOT( play() ), ac, "play" );
    new KAction( i18n( "Pause" ), Pana::icon( "pause" ), 0, ec, SLOT( pause() ), ac, "pause" );
    new KAction( i18n( "Next Track" ), Pana::icon( "next" ), 0, ec, SLOT( next() ), ac, "next" );
    ...
    new Pana::MenuAction( ac );
    new Pana::StopAction( ac );
    new Pana::PlayPauseAction( ac );
    new Pana::AnalyzerAction( ac );
    new Pana::RepeatAction( ac );
    new Pana::RandomAction( ac );
    new Pana::FavorAction( ac );
    new Pana::VolumeAction( ac );

    if( K3bExporter::isAvailable() )
        new Pana::BurnMenuAction( ac );

    if( PanaConfig::playlistWindowSize().isValid() ) {
        // if first ever run, use sizeHint(), and let
        // KWin place us otherwise use the stored values
        resize( PanaConfig::playlistWindowSize() );
        move( PanaConfig::playlistWindowPos() );
    }
}

PlaylistWindow::~PlaylistWindow()
{
    PanaConfig::setPlaylistWindowPos( pos() );  //TODO de XT?
    PanaConfig::setPlaylistWindowSize( size() ); //TODO de XT?
}
...
void PlaylistWindow::init()
{
    ...
    //this function is necessary because Pana::actionCollection() returns our actionCollection
    ...
    dynamicBar->init();
    m_toolbar = new Pana::ToolBar( m_browsers->container(), "mainToolBar" );
#ifndef Q_WS_MAC
    m_toolbar->setShown( PanaConfig::showToolbar() );
#endif
    QWidget *statusbar = new Pana::StatusBar( this );

    KAction* repeatAction = Pana::actionCollection()->action( "repeat" );
    ...
    m_toolsMenu->insertItem( SmallIconSet( Pana::icon( "covermanager" ) ), i18n("&Cover Manager"), Pana::Menu::ID_SHOW_COVER_MANAGER );
    actionCollection()->action("queue_manager")->plug( m_toolsMenu );
    m_toolsMenu->insertItem( SmallIconSet( Pana::icon( "visualizations" ) ), i18n("&Visualizations"), Pana::Menu::ID_SHOW_VIS_SELECTOR );
    m_toolsMenu->insertItem( SmallIconSet( Pana::icon( "equalizer") ), i18n("&Equalizer"), kapp, SLOT( slotConfigEqualizer() ), 0, Pana::Menu::ID_CONFIGURE_EQUALIZER );
    ...
    m_toolsMenu->insertItem( SmallIconSet( Pana::icon( "rescan" ) ), i18n("&Rescan Collection"), Pana::Menu::ID_RESCAN_COLLECTION );

    #if defined HAVE_LIBVISUAL
    m_toolsMenu->setItemEnabled( Pana::Menu::ID_SHOW_VIS_SELECTOR, true );
    #else
    m_toolsMenu->setItemEnabled( Pana::Menu::ID_SHOW_VIS_SELECTOR, false );
    #endif
...
#ifndef Q_WS_MAC
    m_settingsMenu->insertItem( PanaConfig::showToolbar() ? i18n( "Hide Toolbar" ) : i18n("Show Toolbar"), ID_SHOW_TOOLBAR );
    m_settingsMenu->insertItem( PanaConfig::showPlayerWindow() ? i18n("Hide Player &Window") : i18n("Show Player &Window"), ID_SHOW_PLAYERWINDOW );
    ...
    m_menubar->insertItem( i18n( "&Help" ), Pana::Menu::helpMenu() );
    ...
    //<XMLGUI>
    {
        QString xmlFile = Pana::config()->readEntry( "XMLFile", "panaui.rc" );

        // this bug can bite you if you are a pre 1.2 user, we
        // deleted panaui_first.rc, but we must still support it
        // NOTE 1.4.1 we remove panaui_xmms.rc too, so we can only be this ui.rc
        xmlFile = "panaui.rc";
    ...
    //<Browsers>
    {
        ...
        addBrowserMacro( ContextBrowser, "ContextBrowser", i18n("Context"), Pana::icon( "info" ) )
        addBrowserMacro( CollectionBrowser, "CollectionBrowser", i18n("Collection"), Pana::icon( "collection" ) )
        m_browsers->makeDropProxy( "CollectionBrowser", CollectionView::instance() );
        addInstBrowserMacro( PlaylistBrowser, "PlaylistBrowser", i18n("Playlists"), Pana::icon( "playlist" ) )
        ...
        addBrowserMacro( FileBrowser, "FileBrowser", i18n("Files"), Pana::icon( "files" ) )
        //Add Magnatune browser
        addInstBrowserMacro( MagnatuneBrowser, "MagnatuneBrowser", i18n("Magnatune"), Pana::icon( "magnatune" ) )

        new MediaBrowser( "MediaBrowser" );
        if( MediaBrowser::isAvailable() )
        {
            addInstBrowserMacro( MediaBrowser, "MediaBrowser", i18n("Devices"), Pana::icon( "device" ) )
    ...
    Pana::MessageQueue::instance()->sendMessages();
}
...
/**
 * Reload the panaui.rc xml file.
 * mainly just used by pana::Menu
 */
...
/**
 * Create the pana gui from the xml file.
 */
...
{
    switch( PanaConfig::useCustomFonts() )
    {
    case true:
        Playlist::instance()->setFont( PanaConfig::playlistWindowFont() );
        ContextBrowser::instance()->setFont( PanaConfig::contextBrowserFont() );
        break;
    case false:
...
void PlaylistWindow::playLastfmPersonal() //SLOT
{
    ...
    const KURL url( QString( "lastfm://user/%1/personal" )
                    .arg( PanaConfig::scrobblerUsername() ) );
...
void PlaylistWindow::slotMenuActivated( int index ) //SLOT
{
    ...
    default:
        ...
        Pana::Menu::instance()->slotActivated( index );
        break;
    case ID_SHOW_TOOLBAR:
        m_toolbar->setShown( !m_toolbar->isShown() );
        PanaConfig::setShowToolbar( !PanaConfig::showToolbar() );
        m_settingsMenu->changeItem( index, m_toolbar->isShown() ? i18n("Hide Toolbar") : i18n("Show Toolbar") );
        break;
    case ID_SHOW_PLAYERWINDOW:
        PanaConfig::setShowPlayerWindow( !PanaConfig::showPlayerWindow() );
        m_settingsMenu->changeItem( index, PanaConfig::showPlayerWindow() ? i18n("Hide Player &Window") : i18n("Show Player &Window") );
        QTimer::singleShot( 0, kapp, SLOT( applySettings() ) );
        break;
    case Pana::Menu::ID_RESCAN_COLLECTION:
...
{
    m_toolsMenu->setItemEnabled( Pana::Menu::ID_CONFIGURE_EQUALIZER, EngineController::hasEngineProperty( "HasEqualizer" ) );
    m_toolsMenu->setItemEnabled( Pana::Menu::ID_RESCAN_COLLECTION, !ThreadManager::instance()->isJobPending( "CollectionScanner" ) );
}
...
PlaylistWindow::mbAvailabilityChanged( bool isAvailable ) //SLOT
{
    if( isAvailable )
    {
        if( m_browsers->indexForName( "MediaBrowser" ) == -1 )
            m_browsers->addBrowser( "MediaBrowser", MediaBrowser::instance(), i18n( "Media Device" ), Pana::icon( "device" ) );
    }
    else
    {
        if( m_browsers->indexForName( "MediaBrowser" ) != -1 )
