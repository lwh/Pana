...
int MultiTabBarInternal::appendTab( const QPixmap &pic , int id, const QString& text, const QString& identifier )
{
    ...
    tab->setVisible( Pana::config( "BrowserBar" )->readBoolEntry( identifier, true ) );
...
void MultiTabBarInternal::setTabVisible( int id, bool visible )
{
    for ( uint pos = 0;pos < m_tabs.count();pos++ ) {
        if ( m_tabs.at( pos ) ->id() == id ) {
            ...
            Pana::config( "BrowserBar" )->writeEntry( tab->identifier(), tab->visible() );
...
void MultiTabBarTab::updateState()
{
    ...
    if ( m_style != MultiTabBar::KONQSBC ) {
        if ( ( m_style == MultiTabBar::KDEV3 ) || ( m_style == MultiTabBar::KDEV3ICON ) || ( m_style == MultiTabBar::PANA ) || ( isOn() ) ) {
...
{
    if ( m_style == MultiTabBar::PANA ) drawButtonPana( paint );
...
void MultiTabBarTab::drawButtonPana( QPainter *paint )
{
