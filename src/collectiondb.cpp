...
CollectionDB::CollectionDB()
        ...
        , m_autoScoring( true )
        , m_noCover( locate( "data", "pana/images/nocover.png" ) )
        , m_shadowImage( locate( "data", "pana/images/shadow_albumcover.png" ) )
...
#ifdef USE_MYSQL
    if ( PanaConfig::databaseEngine().toInt() == DbConnection::mysql )
        m_dbConnType = DbConnection::mysql;
    else
#endif
#ifdef USE_POSTGRESQL
    if ( PanaConfig::databaseEngine().toInt() == DbConnection::postgresql )
...
CollectionDB::createPermanentIndices()
{
    ...
    //this method is called on each startup of pana
...
CollectionDB::createDragPixmap( const KURL::List &urls, QString textOverRide )
{
    ...
    const int coverW = PanaConfig::coverPreviewSize() > 100 ? 100 : PanaConfig::coverPreviewSize();
    ...
    for ( ; it != urls.end(); ++it )
    {
        ...
        else if( (*it).isLocalFile() )
        {
            ...
            if( mb.compilation() == MetaBundle::CompilationYes )
                artist = QString( "Various_PANA_Artists" ); // magic key for the albumMap!
    ...
    if ( covers > 0 )
    {
        ...
        if ( covers < albums )
        {
            ...
            QImage im( locate( "data","pana/images/more_albums.png" ) );
...
CollectionDB::podcastImage( const QString &remoteURL, const bool withShadow, uint width )
{
    ...
    if( width == 1 )
        width = PanaConfig::coverPreviewSize();

    QString s = findLastfmImage( "Podcast", remoteURL, width );
...
CollectionDB::albumImage( const QString &artist, const QString &album, bool withShadow, uint width, bool* embedded )
{
    ...
    s = findLastfmImage( artist, album, width );

    if( s.isEmpty() )
        s = findLastfmImage( "", album, width ); // handle compilations
...
CollectionDB::makeShadowedImage( const QString& albumImage, bool cache )
{
    ...
    const QString folder = Pana::saveLocation( "covershadow-cache/" );
...
// Lastfm Image
QString
CollectionDB::findLastfmImage( const QString &artist, const QString &album, uint width )
{
    ...
    QCString key = md5sum( artist, album ); // ?PANAFIX - just use the text? filenames might be rough? 
...
CollectionDB::organizeFile( const KURL &src, const OrganizeCollectionDialog &dialog, bool copy )
{
   ...
   if( !localFile )
   {
      ...
      do
      {
         tmp = QString( dialog.folderCombo->currentText() + "/pana-tmp-%1." + extension ).arg( count );
         count++;
...
CollectionDB::applySettings()
{
    ...
    if ( PanaConfig::databaseEngine().toInt() != getDbConnectionType() )
    {
        if ( PanaConfig::databaseEngine().toInt() == DbConnection::mysql )
            m_dbConnType = DbConnection::mysql;
        else if ( PanaConfig::databaseEngine().toInt() == DbConnection::postgresql )
            m_dbConnType = DbConnection::postgresql;
        else m_dbConnType = DbConnection::sqlite;
        recreateConnections = true;
    }
    else if ( PanaConfig::databaseEngine().toInt() == DbConnection::mysql )
    {
        // Using MySQL, so check if MySQL settings were changed
        const MySqlConfig *config =
            static_cast<const MySqlConfig*> ( m_dbConfig );
        if ( PanaConfig::mySqlHost() != config->host() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::mySqlPort() != config->port() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::mySqlDbName() != config->database() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::mySqlUser() != config->username() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::mySqlPassword2() != config->password() )
        {
            recreateConnections = true;
        }
    }
    else if ( PanaConfig::databaseEngine().toInt() == DbConnection::postgresql )
    {
      const PostgresqlConfig *config =
        ...
        if ( PanaConfig::postgresqlHost() != config->host() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::postgresqlPort() != config->port() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::postgresqlDbName() != config->database() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::postgresqlUser() != config->username() )
        {
            recreateConnections = true;
        }
        else if ( PanaConfig::postgresqlPassword() != config->password() )
      {
          recreateConnections = true;
      }
    }
...
CollectionDB::initialize()
{
    ...
    if ( m_dbConnType == DbConnection::mysql )
    {
        QString appVersion = Pana::config( "General Options" )->readEntry( "Version" );
        QString passwd = PanaConfig::mySqlPassword2(); // stored as string type
        ...
        if( passwd.isEmpty() )
        {
            if( appVersion.startsWith( "1.3" ) )
            {
                ...
                passwd = PanaConfig::mySqlPassword(); // stored as password type
                PanaConfig::setMySqlPassword2( passwd );
            }
            else if( appVersion.startsWith( "1.4" ) )
            {
                passwd = Pana::config( "MySql" )->readEntry( "MySqlPassword" ); //read the field as plaintext
                PanaConfig::setMySqlPassword2( passwd ); // store it in plaintext field
            }
        }

        m_dbConfig = new MySqlConfig(
                    PanaConfig::mySqlHost(),
                    PanaConfig::mySqlPort(),
                    PanaConfig::mySqlDbName(),
                    PanaConfig::mySqlUser(),
                    passwd );
    }
    else
#endif
    ...
    {
        QString appVersion = Pana::config( "General Options" )->readEntry( "Version" );
        QString passwd = PanaConfig::postgresqlPassword2();
        ...
        if( passwd.isEmpty() )
        {
            ...
            {
                /// This is because of the encrypted -> plaintext conversion
                passwd = PanaConfig::postgresqlPassword(); // stored as password type
                PanaConfig::setPostgresqlPassword2( passwd );
            }
            else if( appVersion.startsWith( "1.4" ) &&
                   ( appVersion.contains( "beta", false ) ||
                     appVersion.contains( "svn",  false ) ) )
            {
                passwd = Pana::config( "Postgresql" )->readEntry( "PostgresqlPassword" );
                PanaConfig::setPostgresqlPassword2( passwd );
            }
        }

        m_dbConfig = new PostgresqlConfig(
                    PanaConfig::postgresqlHost(),
                    PanaConfig::postgresqlPort(),
                    PanaConfig::postgresqlDbName(),
                    PanaConfig::postgresqlUser(),
                    passwd );
    }
    else
#endif
    {
        m_dbConfig = new SqliteConfig(
                        Pana::config( "Sqlite" )->readPathEntry( "location",
                                        Pana::saveLocation() + "collection.db" ) );
    }
    ...
    if ( !dbConn->isConnected() || !dbConn->isInitialized() )
    {
        ...
        Pana::MessageQueue::instance()->addMessage( dbConn->lastError() );
    }
    else
    {
        if ( !isValid() )
        {
            ...
            Pana::config( "Collection Browser" )->writeEntry( "Database Version", DATABASE_VERSION );
            Pana::config( "Collection Browser" )->writeEntry( "Database Stats Version", DATABASE_STATS_VERSION );
            Pana::config( "Collection Browser" )->writeEntry( "Database Persistent Tables Version", DATABASE_PERSISTENT_TABLES_VERSION );
            Pana::config( "Collection Browser" )->writeEntry( "Database Podcast Tables Version", DATABASE_PODCAST_TABLES_VERSION );    Pana::config( "Collection Browser" )->writeEntry( "Database AFT Version", DATABASE_AFT_VERSION );
        ...
        if ( adminValue( "Database Devices Version" ).isEmpty()
             && Pana::config( "CollectionBrowser" )->readNumEntry( "Database Devices Version", 0 ) == 0 )
        {
            createDevicesTable();
        }
        else if ( adminValue( "Database Devices Version" ).toInt() != DATABASE_DEVICES_VERSION
              || Pana::config( "Collection Browser" )->readNumEntry( "Database Devices Version", 0 ) != DATABASE_DEVICES_VERSION )
        {
            int prev = adminValue( "Database Devices Version" ).toInt();

            if ( prev > DATABASE_DEVICES_VERSION || prev < 0 )
            {
                error() << "Database devices version too new for this version of Pana" << endl;
                exit( 1 );
                //dropDevicesTable();
            }
            else
        ...
        Pana::config( "Collection Browser" )->writeEntry( "Database Devices Version", DATABASE_DEVICES_VERSION );
...
CollectionDB::checkDatabase()
{
    ...
    if ( isValid() )
    {
        ...
        */
        ...
        bool needsUpdate = ( adminValue( "Database Stats Version" ).toInt() != DATABASE_STATS_VERSION
                           || Pana::config( "Collection Browser" )->readNumEntry( "Database Stats Version", 0 ) != DATABASE_STATS_VERSION
                           || Pana::config( "Collection Browser" )->readNumEntry( "Database Version", 0 ) != DATABASE_VERSION
                           || adminValue( "Database Version" ).toInt() != DATABASE_VERSION
                           || Pana::config( "Collection Browser" )->readNumEntry( "Database Persistent Tables Version", 0 ) != DATABASE_PERSISTENT_TABLES_VERSION
                           || adminValue( "Database Persistent Tables Version" ).toInt() != DATABASE_PERSISTENT_TABLES_VERSION
                           || Pana::config( "Collection Browser" )->readNumEntry( "Database Podcast Tables Version", 0 ) != DATABASE_PODCAST_TABLES_VERSION
                           || adminValue( "Database Podcast Tables Version" ).toInt() != DATABASE_PODCAST_TABLES_VERSION
                           || Pana::config( "Collection Browser" )->readNumEntry( "Database AFT Version", 0 ) != DATABASE_AFT_VERSION
        ...
        if ( needsUpdate )
        {
            ...
            /* TODO: remove the standard window controls from the dialog window, the user should not be able
                     ...
                     add additional text, e.g. Pana is currently updating your database. This may take a while.
                     Please wait.
            ...
            */
            ...
            if ( Pana::config( "Collection Browser" )->readNumEntry( "Database Version", 0 ) != DATABASE_VERSION
                 || adminValue( "Database Version" ).toInt() != DATABASE_VERSION )
            {
    ...
    Pana::config( "Collection Browser" )->writeEntry( "Database Stats Version", DATABASE_STATS_VERSION );
    Pana::config( "Collection Browser" )->writeEntry( "Database Persistent Tables Version", DATABASE_PERSISTENT_TABLES_VERSION );
    Pana::config( "Collection Browser" )->writeEntry( "Database Podcast Tables Version", DATABASE_PODCAST_TABLES_VERSION );
    Pana::config( "Collection Browser" )->writeEntry( "Database AFT Version", DATABASE_AFT_VERSION );
...
CollectionDB::updateGroupBy()
{
    ...
    if (!version) // an even older update
       version = Pana::config( "Collection Browser" )->readNumEntry( "Database Version", 0 );

    if ( version && version < 32 )
    {
        KConfig* config = Pana::config( "Collection Browser" );
...
CollectionDB::updateStatsTables()
{
          ...
          || Pana::config( "Collection Browser" )->readNumEntry( "Database Stats Version", 0 ) != DATABASE_STATS_VERSION )
    {
        debug() << "Different database stats version detected! Stats table will be updated or rebuilt." << endl;
            ...
            if( getType() == DbConnection::sqlite && QFile::exists( Pana::saveLocation()+"collection.db" ) )
            {
                debug() << "Creating a backup of the database in "
                        << Pana::saveLocation()+"collection-backup.db" << '.' << endl;

                bool copied = KIO::NetAccess::file_copy( Pana::saveLocation()+"collection.db",
                                                         Pana::saveLocation()+"collection-backup.db",
        ...
        /* If config returns 3 or lower, it came from an Pana version that was not aware of
           admin table, so we can't trust this table at all */
        if( !prev || ( Pana::config( "Collection Browser" )->readNumEntry( "Database Stats Version", 0 )
                  && Pana::config( "Collection Browser" )->readNumEntry( "Database Stats Version", 0 ) <= 3  ) )
            prev = Pana::config( "Collection Browser" )->readNumEntry( "Database Stats Version", 0 );
        ...
        else        //Incrementally update the stats table to reach the present version
        {
            ...
            if( prev < 9 )
            {
                ...
                MountPointManager::instance()->setCollectionFolders( Pana::config( "Collection" )->readPathListEntry( "Collection Folders" ) );
            ...
            {
                error() << "Database statistics version too new for this version of Pana. Quitting..." << endl;
                exit( 1 );
            }
        }
    }
...
CollectionDB::updatePersistentTables()
{
    ...
    if ( PersistentVersion.isEmpty() )
    {
        ...
        debug() << "Detected old schema for tables with important data. Pana will convert the tables, ignore any \"table already exists\" errors." << endl;
        createPersistentTables();
        /* Copy lyrics */
    ...
    else
    {
        ...
        if ( PersistentVersion.toInt() > 19 || PersistentVersion.toInt() < 0 )
        {
            ...
            {
                error() << "There is a bug in Pana: instead of destroying your valuable"
...
CollectionDB::loadHashFile( const QCString& hash, uint width )
{
    ...
    if ( width == 0 ) {
        ...
        if ( width == 1 ) width = PanaConfig::coverPreviewSize();
        QCString widthKey = makeWidthKey( width );
...
{
    return QDir( Pana::saveLocation( "albumcovers/large/" ) );
}
...
{
    return QDir( Pana::saveLocation( "albumcovers/tagcover/" ) );
}
...
{
    return QDir( Pana::saveLocation( "albumcovers/cache/" ) );
}
...
MySqlConnection::setMysqlError()
{
    m_error = i18n("MySQL reported the following error:<br>") + mysql_error(m_db)
            + i18n("<p>You can configure MySQL in the Collection section under Settings->Configure Pana</p>");
}
#endif
...
//     if ( config->username().isEmpty() )
//         pApp->slotConfigPana("Postgresql");
...
void PostgresqlConnection::setPostgresqlError()
{
    m_error = i18n("Postgresql reported the following error:<br>") + PQerrorMessage(m_db)
            + i18n("<p>You can configure Postgresql in the Collection section under Settings->Configure Pana</p>");
}
#endif
...
QueryBuilder::valForFavoriteSorting() {
    ...
    if ( !PanaConfig::useScores() && !PanaConfig::useRatings() )
        favSortBy = valPlayCounter;
    else if( !PanaConfig::useRatings() )
        favSortBy = valScore;
    return favSortBy;
}
