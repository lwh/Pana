...
Pana::Slider::Slider( Qt::Orientation orientation, QWidget *parent, uint max )
...
void
Pana::Slider::wheelEvent( QWheelEvent *e )
{
...
void
Pana::Slider::mouseMoveEvent( QMouseEvent *e )
{
    if ( m_sliding )
    {
...
void
Pana::Slider::slideEvent( QMouseEvent *e )
{
...
void
Pana::Slider::mousePressEvent( QMouseEvent *e )
{
    m_sliding   = true;
...
void
Pana::Slider::mouseReleaseEvent( QMouseEvent* )
{
...
void
Pana::Slider::setValue( int newValue )
{
...
Pana::PrettySlider::PrettySlider( Qt::Orientation orientation, SliderMode mode,
                                    QWidget *parent, uint max )
    : Pana::Slider( orientation, parent, max )
    , m_mode( mode )
...
void
Pana::PrettySlider::mousePressEvent( QMouseEvent *e )
{
    Pana::Slider::mousePressEvent( e );

    slideEvent( e );
}

void
Pana::PrettySlider::slideEvent( QMouseEvent *e )
{
...
void
Pana::PrettySlider::paintEvent( QPaintEvent *e )
{
    ...
    if( !m_showingMoodbar )
      {
        p.translate( 0, MARGIN );
          p.setPen( Pana::ColorScheme::Foreground );
          p.fillRect( 0, 0, pos, h, QColor( Pana::ColorScheme::Background ) );
          p.drawRect( 0, 0, w, h );
        p.translate( 0, -MARGIN );
      }
    else
...
void
Pana::PrettySlider::moodbarJobEvent( int newState )
{
...
void
Pana::PrettySlider::slotMoodbarPrefs( bool show, bool moodier, int alter, bool withMusic )
{
...
void
Pana::PrettySlider::newBundle( const MetaBundle &bundle )
{
    m_bundle = bundle;
    m_bundle.detach();
...
QSize
Pana::PrettySlider::minimumSizeHint() const
{
    return sizeHint();
}

QSize
Pana::PrettySlider::sizeHint() const
{
    constPolish();
...
Pana::VolumeSlider::VolumeSlider( QWidget *parent, uint max )
    : Pana::Slider( Qt::Horizontal, parent, max )
    , m_animCount( 0 )
    , m_animTimer( new QTimer( this ) )
    , m_pixmapInset( QPixmap( locate( "data","pana/images/volumeslider-inset.png" ) ) )
{
    setWFlags( getWFlags() | WNoAutoErase );
    setFocusPolicy( QWidget::NoFocus );

    // BEGIN Calculate handle animation pixmaps for mouse-over effect
    QImage pixmapHandle    ( locate( "data","pana/images/volumeslider-handle.png" ) );
    QImage pixmapHandleGlow( locate( "data","pana/images/volumeslider-handle_glow.png" ) );
...
void
Pana::VolumeSlider::generateGradient()
{
    //QImage temp( locate( "data","pana/images/volumeslider-gradient.png" ) );
    //KIconEffect::colorize( temp, colorGroup().highlight(), 1.0 );

    const QPixmap temp( locate( "data","pana/images/volumeslider-gradient.png" ) );
...
void
Pana::VolumeSlider::slotAnimTimer() //SLOT
{
    if ( m_animEnter ) {
        m_animCount++;
        repaint( false );
...
void
Pana::VolumeSlider::mousePressEvent( QMouseEvent *e )
{
...
void
Pana::VolumeSlider::contextMenuEvent( QContextMenuEvent *e )
{
    KPopupMenu menu;
...
void
Pana::VolumeSlider::slideEvent( QMouseEvent *e )
{
    QSlider::setValue( QRangeControl::valueFromPosition( e->pos().x(), width()-2 ) );
}

void
Pana::VolumeSlider::wheelEvent( QWheelEvent *e )
{
    const uint step = e->delta() / Pana::VOLUME_SENSITIVITY;
...
void
Pana::VolumeSlider::paintEvent( QPaintEvent * )
{
    QPixmap buf( size() );
...
void
Pana::VolumeSlider::hideEvent( QHideEvent* )
{
    setBackgroundMode( PaletteBackground ); // Required to prevent erasing
}

void
Pana::VolumeSlider::showEvent( QShowEvent* )
{
...
void
Pana::VolumeSlider::enterEvent( QEvent* )
{
    m_animEnter = true;
    m_animCount = 0;
...
void
Pana::VolumeSlider::leaveEvent( QEvent* )
{
...
void
Pana::VolumeSlider::paletteChange( const QPalette& )
{
    generateGradient();
}
