...
PluginManager::createFromQuery( const QString &constraint )
{
    ...
    for ( uint i = 0; i < offers.count(); i++ ) {
        if ( offers[i]->property( "X-KDE-Pana-rank" ).toInt() > rank )
            current = i;
    }
...
PluginManager::showAbout( const QString &constraint )
{
    ...
    str += body.arg( i18n( "Authors" ),             s->property( "X-KDE-Pana-authors" ).toStringList().join( "\n" ) );
    str += body.arg( i18n( "Email" ),               s->property( "X-KDE-Pana-email" ).toStringList().join( "\n" ) );
    str += body.arg( i18n( "Version" ),             s->property( "X-KDE-Pana-version" ).toString() );
    str += body.arg( i18n( "Framework Version" ),   s->property( "X-KDE-Pana-framework-version" ).toString() );
...
PluginManager::dump( const KService::Ptr service )
{
      ...
      debug() << "X-KDE-Pana-plugintype       : " << service->property( "X-KDE-Pana-plugintype" ).toString() << endl;
      debug() << "X-KDE-Pana-name             : " << service->property( "X-KDE-Pana-name" ).toString() << endl;
      debug() << "X-KDE-Pana-authors          : " << service->property( "X-KDE-Pana-authors" ).toStringList() << endl;
      debug() << "X-KDE-Pana-rank             : " << service->property( "X-KDE-Pana-rank" ).toString() << endl;
      debug() << "X-KDE-Pana-version          : " << service->property( "X-KDE-Pana-version" ).toString() << endl;
      debug() << "X-KDE-Pana-framework-version: " << service->property( "X-KDE-Pana-framework-version" ).toString() << endl;
}
