...
void EqDialog::init()
{
    ...
    if(!KApplication::dcopClient()->call("pana","player","equalizerEnabled()", send_data, reply_type, reply_data,true,1000));
    //kdDebug() << "called" << endl;
...
void EqDialog::eqGroupBox_toggled( bool eqEnabled)
{
        ...
        arg << eqEnabled;
        KApplication::dcopClient()->send("pana", "player", "setEqualizerEnabled(bool)" , data);
}
