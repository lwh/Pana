#!/usr/bin/env ruby
#
# Pana Script for fetching song lyrics from http://lyrc.com.ar.
# Ported from Pana's contextbrowser.cpp.
#
...
def showLyrics( lyrics )
    system("dcop", "pana", "contextbrowser", "showLyrics", lyrics)
end
...
def fetchLyrics( artist, title, url )
    ...
    proxy_pass = nil
    if ( @proxy == nil )
        @proxy = `dcop pana script proxyForUrl #{@page_url.shellquote}`
    end
...
#         error "HTTP Error: #{response.message}"
        `dcop pana contextbrowser showLyrics ""`
        return
    end
...
loop do
    ...
    case command
        when "configure"
            `dcop pana playlist popupMessage "This script does not require any configuration."`
