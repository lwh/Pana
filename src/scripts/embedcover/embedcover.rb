#!/usr/bin/env ruby
#
# Pana Script for embedding album cover images in MP3 files.
#
...
def cleanup()
  `dcop pana script removeCustomMenuItem #{MenuItemName}`
end
...
`dcop pana script addCustomMenuItem #{MenuItemName}`
...
loop do
  ...
  when "configure"
    ...
    `dcop pana playlist popupMessage "#{msg}"`
  ...
  when "customMenuClicked"
    if message.include?( MenuItemName )
      ...
      args.each() do |arg|
        ...
        file_relative = `dcop pana collection relativePath "#{file}"`.chomp

        # Query is two parts, first ID, then name
        artist_id = `dcop pana collection query "SELECT DISTINCT artist FROM tags WHERE url = '#{sql_escape( file_relative )}'"`.chomp
        artist    = `dcop pana collection query "SELECT DISTINCT artist.name FROM artist WHERE id = '#{artist_id}'"`.chomp

        album_id  = `dcop pana collection query "SELECT DISTINCT album FROM tags WHERE url = '#{sql_escape( file_relative )}'"`.chomp
        album     = `dcop pana collection query "SELECT DISTINCT album.name FROM album WHERE id = '#{album_id}'"`.chomp
        ...
        if artist_id.empty?() and album_id.empty?()
          `dcop pana playlist popupMessage "EmbedCover Error: This track is not in your Collection."`
          next
        end

        md5sum = MD5.hexdigest( "#{artist.downcase()}#{album.downcase()}" )
        imagefolder = "#{ENV['HOME']}/.kde/share/apps/pana/albumcovers/large/"
        ...
        unless FileTest.exist?( image )
          ...
          images = `dcop pana collection query #{sql}`.split( "\n" )

          # FIXME select best image from array, like CollectionDB does
          image = images.first()

          if image == nil or not FileTest.exist?( image )
            `dcop pana playlist popupMessage "EmbedCover: No image found for this track."`
            next
          end
        end
        ...
        if $?.success?()
          `dcop pana playlist popupMessage "EmbedCover has successfully embedded the image."`
        else
          reg = Regexp.new( "Error:.*", Regexp::MULTILINE )
          errormsg = reg.match( output )

          `dcop pana playlist popupMessage "EmbedCover #{errormsg}"`
        end
      end
    end
  end
