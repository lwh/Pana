#!/usr/bin/env ruby
#
# Pana-Script for fixing VBR encoded mp3 files without XING header. Calculates the real
...
def cleanup()
    system("dcop", "pana", "script", "removeCustomMenuItem", MenuItemName)
    # use unlink ??!
...
system("dcop", "pana", "script", "addCustomMenuItem", MenuItemName)
...
loop do
    ...
    case command
        when "configure"
            ...
            system("dcop", "pana", "playlist", "popupMessage", msg)
        ...
        when "customMenuClicked"
            if message.include?( MenuItemName )
                ...
                args.each() do |arg|
                    ...
                    system("dcop", "pana", "playlist", "shortStatusMessage", "Mp3Fixer is analyzing the file '#{filename}'...")
                    ...
                    if $?.success?()
                        ...
                        report.gsub!( "\n", "<BR/>" )
                        system("dcop", "pana", "playlist", "popupMessage", report)
                        ...
                        system("dcop", "pana", "playlist", "popupMessage", "Mp3Fixer #{errormsg}")
                    end
                end
                ...
                system("dcop", "pana", "collection", "scanCollectionChanges")

                # Refresh the playlist
                system("dcop", "pana", "playlist", "saveM3u", Dir.getwd() + "/mp3fixer_playlist.m3u", "false")
                system("dcop", "pana", "playlist", "clearPlaylist")
                system("dcop", "pana", "playlist", "addMedia", Dir.getwd() + "/mp3fixer_playlist.m3u")
            end
    end
end
