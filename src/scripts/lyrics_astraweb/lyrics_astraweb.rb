#!/usr/bin/env ruby
#
# Pana Script for fetching song lyrics from http://lyrics.astraweb.com.
#
...
#         lyrics = "HTTP Error: #{response.message}"
        system("dcop", "pana", "contextbrowser", "showLyrics")
        return
    end
...
def fetchLyricsByUrl( url )
    ...
    lyrics = /(<font face=sans size=2>)(.*)(<br><br><br><center>)/.match( body )[2].to_s()
...
loop do
    ...
    case command
        when "configure"
            system("dcop", "pana", "playlist", "popupMessage", 'This script does not require any configuration.')
