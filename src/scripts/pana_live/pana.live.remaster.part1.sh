...
############################################################################
# Disassemble an Pana LiveCD iso file into a specified directory
...
#			echo "comparing" $free "to 1572864"
            if [[ $free -gt 1572864 ]] ; then
                ...
                dcop --all-users pana playlist popupMessage "Not enough free space. Please select another folder."
                break
            fi
        else
...
if [[  `cat /proc/filesystems | grep squash | wc -l` = 0 ]]; then
	...
	if [[  `cat /proc/filesystems | grep squash | wc -l` = 0 ]]; then
    	 ...
    	 dcop --all-users pana playlist popupMessage "You do not have squashfs support enabled. You need to have a patched kernel with squashfs. You can find more info about squashfs, and how to patch your kernel, here: http://tldp.org/HOWTO/SquashFS-HOWTO/"
	
		rm -rf $WORK/mklivecd
	fi
fi

mkdir $WORK/pana.livecd/
mount -o loop -t squashfs $WORK/mklivecd/livecd/livecd.sqfs $WORK/pana.livecd/

# gotta copy it locally so the user can add files to it

mkdir $WORK/pana.live/
dcop --all-users pana playlist shortStatusMessage "Copying files now. Please be patient, this step takes a long time."
#echo
#echo "Please wait, copying in progress."
#echo
cp -a $WORK/pana.livecd/* $WORK/pana.live/
umount $WORK/pana.livecd/
rmdir $WORK/pana.livecd

mkdir $WORK/pana.live/home/pana/.kde/share/apps/pana/playlists
chown 500:500 $WORK/pana.live/home/pana/.kde/share/apps/pana/playlists
find $WORK/pana.live/home/pana/.kde/share/apps/ -type d -print0 | xargs -0 chmod +x
chmod -R 777 $WORK/pana.live/music/
chmod -R 777 $WORK/pana.live/home/pana/.kde/share/apps/pana
#chmod -R 777 $WORK/mklivecd/

dcop --all-users pana playlist popupMessage "Copying done. To add music to the Pana livecd, select the tracks you wish to add in the playlist, and select \"Add to livecd\" from the right click menu. Please do not add more than about 380 mb, as then the resulting ISO will be too large to fit on a CD-ROM. Once you are done, select Create Remastered CD. Enjoy!"
