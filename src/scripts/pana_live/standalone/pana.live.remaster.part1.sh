...
kdialog --title "pana livecd remaster" --yesno "Welcome to the pana live cd remaster utility. The first step is to select the iso image, would you like to continue."
...
if [[  `cat /proc/filesystems | grep squash | wc -l` = 0 ]]; then
	...
	if [[  `cat /proc/filesystems | grep squash | wc -l` = 0 ]]; then
    	...
    	kdialog --title "pana livecd remaster" --error "You do not have squashfs support enabled. You need to have a patched kernel with squashfs. You can find more info about squashfs, and how to patch your kernel, here: http://tldp.org/HOWTO/SquashFS-HOWTO/"
...
kdialog --title "pana livecd remaster" --msgbox "Copying files now. Please be patient, this step takes a long time."
echo
...
kdialog --title "pana livecd remaster" --msgbox "Copying done. To add music to the pana livecd, place additional music in /tmp/pana.live/music/ Please do not add more than about 380 mb, as then the resulting ISO will be too large to fit on a CD-ROM. Once you are done, run the pana.live.remaster.part2.sh script and you are finished!."
