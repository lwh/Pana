...
############################################################################
# Python wrapper script for running the Pana LiveCD remastering scripts
# from within Pana.  Based on the Python-Qt template script for Pana
...
class ConfigDialog ( QDialog ):
    ...
    def __init__( self ):
        ...
        self.setCaption("Pana Live! Configuration")
        ...
        self.isopath.setText( "Path to Pana Live! iso" )
...
#        self.connect( self.ok, SIGNAL( "clicked()" ), self.destroy )
    ...
    def unpack( self ):
        ...
        kdedir = fd.readline()
        kdedir = kdedir.strip()
        scriptdir = kdedir + "/share/apps/pana/scripts/pana_live"
        fd.close()

        path, iso = self.readConfig()
        os.system("kdesu -t sh %s/pana.live.remaster.part1.sh %s %s" % (scriptdir, path, iso))
        #os.wait()
...
class Remasterer( QApplication ):
    ...
    def __init__( self, args ):
        ...
        os.system("dcop pana script removeCustomMenuItem \"Pana live\" \"Add playlist to livecd\"")
        os.system("dcop pana script removeCustomMenuItem \"Pana live\" \"Add selected to livecd\"")
        os.system("dcop pana script removeCustomMenuItem \"Pana live\" \"Create Remastered CD\"")
        os.system("dcop pana script removeCustomMenuItem \"Pana live\" \"Clear Music on livecd\"")
        
        os.system("dcop pana script addCustomMenuItem \"Pana live\" \"Add playlist to livecd\"")
        os.system("dcop pana script addCustomMenuItem \"Pana live\" \"Add selected to livecd\"")
        os.system("dcop pana script addCustomMenuItem \"Pana live\" \"Create Remastered CD\"")
        os.system("dcop pana script addCustomMenuItem \"Pana live\" \"Clear Music on livecd\"")
...
# events from Pana:
    ...
    def clearCD( self ):
        ...
        os.system("rm -rf %s/pana.live/music/* %s/pana.live/playlist/* %s/pana.live/home/pana/.kde/share/apps/pana/current.xml" % (path, path, path))
    ...
    def copyPlaylist( self ):
        ...
        path, iso = self.dia.readConfig()
        if path == "":
            os.system("dcop pana playlist popupMessage 'Please run configure first.'")
            return

        tmpfileloc = os.tmpnam()
        os.system("dcop pana playlist saveM3u '%s' false" % tmpfileloc)
        ...
        os.system("kfmclient copy %s file://%s/pana.live/music/" % (files, path))

        import random
        suffix = random.randint(0,10000)
#        os.system("mkdir %s/pana.live/home/pana/.kde/share/apps/pana/playlists/" % path)
        m3uOut = open("/tmp/pana.live.%s.m3u" % suffix, 'w')
        ...
        os.system("mv /tmp/pana.live.%s.m3u %s/pana.live/playlist/" % (suffix,path))
        os.system("rm /tmp/pana.live.%s.m3u" % suffix)
    ...
    def copyTrack( self, menuEvent ):
        ...
        path,iso = self.dia.readConfig()
        if path == "":
            os.system("kdialog --sorry 'You have not specified where the Pana live iso is. Please click configure and do so first.'")
        else:
            # get the list of files. yes, its ugly. it works though.
            #files =  event.split(":")[-1][2:-1].split()[2:]
            #trying out a new one 
         #files = event.split(":")[-1][3:-2].replace("\"Pana live!\" \"add to livecd\" ", "").split("\" \"")
            #and another
          
            files = event.replace("customMenuClicked: Pana live Add selected to livecd", "").split()

            allfiles = ""
            for file in files:
                allfiles += file + " "
            allfiles = allfiles.strip()
            os.system("kfmclient copy %s file://%s/pana.live/music/" % (allfiles, path))
    ...
    def createCD( self ):
        ...
        path,iso = self.dia.readConfig()
        if path == "":
            os.system("kdialog --sorry 'You have not configured Pana live! Please run configure.")
        ...
        os.system("kdesu sh %s/pana.live.remaster.part2.sh %s" % (scriptdir, path))

        fd = open("/tmp/pana.script", 'r')
        y = fd.readline()
        y = y.strip()
