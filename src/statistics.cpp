...
StatisticsList::renderView()
{
    ...
    m_newestItem->setSubtext( i18n("First played %1").arg( Pana::verboseTimeSince( firstPlay ) ) );

    m_trackItem     ->setIcon( Pana::icon("track") );
    m_mostplayedItem->setIcon( Pana::icon("mostplayed") );
    m_artistItem    ->setIcon( Pana::icon("artist") );
    m_albumItem     ->setIcon( Pana::icon("album") );
    m_genreItem     ->setIcon( Pana::icon("favourite_genres") );
    m_newestItem    ->setIcon( Pana::icon("clock") );
}
...
StatisticsList::expandInformation( StatisticsItem *item, bool refresh )
{
    ...
    else if( item == m_newestItem )
    {
        ...
        for( uint i=0; i < newest.count(); i += qb.countReturnValues() )
        {
            ...
            QString subtext = i18n("Added: %1").arg( Pana::verboseTimeSince( added ) );
            m_last = new StatisticsDetailedItem( name, subtext, item, m_last );
...
StatisticsDetailedItem::getSQL()
{
    ...
    Pana::albumArtistTrackFromUrl( url(), artist, album, track );
...
StatisticsDetailedItem::getURLs()
{
    ...
    QString query = QString::null;
    QString artist, album, track;   // track is unused here
    Pana::albumArtistTrackFromUrl( m_url, artist, album, track );
