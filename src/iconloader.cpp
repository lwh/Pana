...
QString
Pana::icon( const QString& name ) //declared in pana.h
{
    // We map our Pana icon theme names to system icons, instead of using the same
    ...
    static QMap<QString, QString> panaMap;
    if( panaMap.empty() ) {
        panaMap["queue_track"]          = "fastforward";
        panaMap["dequeue_track"]        = "rewind";
    }

    if( iconMap.contains( name ) )
    {
        if( PanaConfig::useCustomIconTheme() )
        {
            if( panaMap.contains( name ) )
                return QString( "pana_" ) + panaMap[name];
            return QString( "pana_" ) + name;
        }
        else
            return iconMap[name];
    }
