    ...
    , m_folder( isFolder )
{
    ...
    setPixmap( 0, SmallIcon( Pana::icon( "files2" ) ) );
    setText( 0, t );
}
    ...
    , m_folder( isFolder )
{
    setDragEnabled( false );
    setRenameEnabled( 0, isFolder );
    setPixmap( 0, SmallIcon( Pana::icon( "files" ) ) );
    setText( 0, t );
}
    ...
    if ( tname == "category" )
    {
        ...
        for( QDomNode n = xml.firstChild() ; !n.isNull(); n = n.nextSibling() )
        {
            ...
            else if ( e.tagName() == "podcast" )
            {
                ...
                QString xmlLocation = Pana::saveLocation( "podcasts/" );
...
PlaylistCategory::showContextMenu( const QPoint &position )
{
    ...
    if( isFolder() ) {
        menu.insertItem( SmallIconSet( Pana::icon("edit") ), i18n( "&Rename" ), RENAME );
        menu.insertItem( SmallIconSet( Pana::icon("remove") ), i18n( "&Delete" ), REMOVE );
        menu.insertSeparator();
    }

    if( parentCat == static_cast<QListViewItem*>( PlaylistBrowser::instance()->m_playlistCategory) )
    {
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("Create Playlist..."), PLAYLIST );
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("Import Playlist..."), PLAYLIST_IMPORT );
    }

    else if( parentCat == static_cast<QListViewItem*>(PlaylistBrowser::instance()->m_smartCategory) )
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("New Smart Playlist..."), SMART );

    else if( parentCat == static_cast<QListViewItem*>(PlaylistBrowser::instance()->m_dynamicCategory) )
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("New Dynamic Playlist..."), DYNAMIC );

    else if( parentCat == static_cast<QListViewItem*>(PlaylistBrowser::instance()->m_streamsCategory) )
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("Add Radio Stream..."), STREAM );

    else if( parentCat == static_cast<QListViewItem*>(PlaylistBrowser::instance()->m_lastfmCategory) )
    {
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("Add Last.fm Radio..."), LASTFM );
        menu.insertItem( SmallIconSet(Pana::icon( "add_playlist" )), i18n("Add Custom Last.fm Radio..."), LASTFMCUSTOM );
    }

    else if( parentCat == static_cast<QListViewItem*>(PlaylistBrowser::instance()->m_podcastCategory) )
    {
        isPodcastFolder = true;
        menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n("Add Podcast..."), PODCAST );
        menu.insertItem( SmallIconSet( Pana::icon( "refresh" ) ), i18n("Refresh All Podcasts"), REFRESH );
        menu.insertSeparator();
        menu.insertItem( SmallIconSet( Pana::icon( "configure" ) ), i18n( "&Configure Podcasts..." ), CONFIG );
        if( parentCat->childCount() == 0 )
            menu.setItemEnabled( CONFIG, false );
        if( parentCat == this )
            menu.insertItem( SmallIconSet( Pana::icon( "configure" ) ), i18n("Scan Interval..."), INTERVAL );
    }

    menu.insertSeparator();
    menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n("Create Sub-Folder"), CREATE );
...
PlaylistEntry::PlaylistEntry( QListViewItem *parent, QListViewItem *after, const KURL &url, int tracks, int length )
    ...
    , m_loaded( false )
    , m_dynamic( false )
    , m_loading1( new QPixmap( locate("data", "pana/images/loading1.png" ) ) )
    , m_loading2( new QPixmap( locate("data", "pana/images/loading2.png" ) ) )
    , m_lastTrack( 0 )
{
    ...
    setPixmap( 0, SmallIcon( Pana::icon( "playlist" ) ) );
...
void PlaylistEntry::stopAnimation()
{
    ...
    m_dynamic ?
        setPixmap( 0, SmallIcon( Pana::icon( "favorites" ) ) ):
        setPixmap( 0, SmallIcon( Pana::icon( "playlist" ) ) );
}
...
void PlaylistEntry::showContextMenu( const QPoint &position )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "&Load" ), LOAD );
    menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), APPEND );
    menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Tracks" ), QUEUE );

    if( MediaBrowser::isAvailable() )
    {
        menu.insertSeparator();
        menu.insertItem( SmallIconSet( Pana::icon( "device" ) ),
    ...
    menu.insertSeparator();
    menu.insertItem( SmallIconSet( Pana::icon("edit") ), i18n( "&Rename" ), RENAME );
    menu.insertItem( SmallIconSet( Pana::icon("remove_from_playlist") ), i18n( "&Delete" ), DELETE );
...
void PlaylistEntry::slotPostRenameItem( const QString newName )
{
    ...
    QString newPath = fileDirPath( oldPath ) + newName + '.' + Pana::extension( oldPath );
...
void PlaylistTrackItem::showContextMenu( const QPoint &position )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "&Load" ), LOAD );
    menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), APPEND );
    menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Track" ), QUEUE );


    menu.insertSeparator();

    menu.insertItem( SmallIconSet( Pana::icon( "burn" ) ), i18n("Burn to CD"), BURN );
    menu.setItemEnabled( BURN, K3bExporter::isAvailable() && url().isLocalFile() );

    menu.insertSeparator();

    menu.insertItem( SmallIconSet( Pana::icon( "remove_from_playlist" ) ), i18n( "&Remove" ), REMOVE );
...
StreamEntry::showContextMenu( const QPoint &position )
{
    ...
    if( isKept() )
    {
        menu.insertItem( SmallIconSet( Pana::icon("edit") ), i18n( "E&dit" ), EDIT );
        menu.insertItem( SmallIconSet( Pana::icon("remove_from_playlist") ), i18n( "&Delete" ), REMOVE );
    }
    else
        menu.insertItem( SmallIconSet( Pana::icon( "info" ) ), i18n( "Show &Information" ), EDIT );
...
DynamicEntry::DynamicEntry( QListViewItem *parent, QListViewItem *after, const QString &name )
        ...
        , DynamicMode( name )
{
    setPixmap( 0, SmallIcon( Pana::icon( "dynamic" ) ) );
    setDragEnabled( true );
}
        ...
        , m_settingsValid( false )
{
    ...
    setPixmap( 0, SmallIcon( Pana::icon( "podcast" ) ) );
...
PodcastChannel::setDOMSettings( const QDomNode &channelSettings )
{
    ...
    QString t = title();
    if( save.isEmpty() )
        save = Pana::saveLocation( "podcasts/" + Pana::vfatPath( t ) );
...
PodcastChannel::setSettings( PodcastSettings *newSettings )
{
    ...
    if( saveLocation() != newSettings->saveLocation() )
    {
        ...
        if( !copyList.isEmpty() )
        {
            ...
            Pana::StatusBar::instance()->newProgressOperation( m_podcastMoveJob )
...
PodcastChannel::fetch()
{
    ...
    Pana::StatusBar::instance()->newProgressOperation( m_podcastJob )
...
PodcastChannel::fetchResult( KIO::Job* job ) //SLOT
{
    ...
    if ( job->error() != 0 )
    {
        Pana::StatusBar::instance()->shortMessage( i18n( "Unable to connect to Podcast server." ) );
    ...
    {
        Pana::StatusBar::instance()->shortMessage( i18n("Podcast returned invalid data.") );
    ...
    if( type.isNull() || type.toElement().attribute( "version" ) != "2.0" )
    {
        ...
        if( type.isNull() )
        {
            Pana::StatusBar::instance()->shortMessage( i18n("Sorry, only RSS 2.0 or Atom feeds for podcasts!") );
...
{
    if( n )
        setPixmap( 0, SmallIcon( Pana::icon( "podcast2" ) ) );
...
PodcastChannel::setXml( const QDomNode &xml, const int feedType )
{
    ...
    {
        setNew();
        Pana::StatusBar::instance()->shortMessage( i18n("New podcasts have been retrieved!") );
    }
}
...
PodcastChannel::showContextMenu( const QPoint &position )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Tracks" ), QUEUE );
    menu.insertSeparator();
    menu.insertItem( SmallIconSet( Pana::icon( "remove" ) ), i18n( "&Delete" ), DELETE );
    menu.insertItem( SmallIconSet( Pana::icon( "refresh" ) ), i18n( "&Check for Updates" ), RESCAN );
    menu.insertItem( SmallIconSet( Pana::icon( "artist" ) ), i18n( "Mark as &Listened" ), LISTENED );
    menu.insertItem( SmallIconSet( Pana::icon( "artist" ) ), i18n( "Mark as &New" ), NEW );
    menu.insertItem( SmallIconSet( Pana::icon( "configure" ) ), i18n( "&Configure..." ), CONFIG );
...
PodcastEpisode::downloadMedia()
{
    ...
    Pana::StatusBar::instance()->newProgressOperation( m_podcastEpisodeJob )
...
void PodcastEpisode::downloadResult( KIO::Job * transferJob )
{
    ...
    {
        Pana::StatusBar::instance()->shortMessage( i18n( "Media download aborted, unable to connect to server." ) );
...
PodcastEpisode::paintCell( QPainter *p, const QColorGroup &cg, int column, int width, int align )
{
    ...
    if( fmName.width( name ) > _width )
    {
        ...
        name = Pana::decapitateString( name, static_cast<PodcastChannel *>(m_parent)->title() );
        if( fmName.width( name ) > _width )
            name = KStringHandler::rPixelSqueeze( name, pBuf.fontMetrics(), _width );
    }
...
PodcastEpisode::showContextMenu( const QPoint &position )
{
    ...
    if( offers.empty() || (offers.size()==1 && offers.first()->name()=="Pana") )
    {
        menu.insertItem( SmallIconSet( Pana::icon( "run" ) ), i18n( "&Open With..."), OPEN_WITH );
    }
    else
    {
        int i = 1;
        ...
        openMenu->insertItem( SmallIconSet( Pana::icon( "run" ) ), i18n( "&Other..."), OPEN_WITH );
        menu.insertItem( SmallIconSet( Pana::icon( "run" ) ), i18n("&Open With"), openMenu, OPEN_WITH );
    }
    ...
    menu.insertSeparator();
    menu.insertItem( SmallIconSet( Pana::icon( "download" ) ), i18n( "&Download Media" ), GET );
    menu.insertItem( SmallIconSet( Pana::icon( "attach" ) ), i18n( "&Associate with Local File" ), ASSOCIATE );
    menu.insertItem( SmallIconSet( Pana::icon( "artist" ) ),   i18n( "Mark as &Listened" ),  LISTENED );
    menu.insertItem( SmallIconSet( Pana::icon( "artist" ) ),   i18n( "Mark as &New" ),  NEW );
    menu.insertItem( SmallIconSet( Pana::icon("remove") ), i18n( "De&lete Downloaded Podcast" ), DELETE );
...
class AssociatePodcastDialog : public KDialogBase
{
    ...
    AssociatePodcastDialog( PodcastEpisode *item )
        : KDialogBase( Pana::mainWindow(), "associatepodcastdialog", true, i18n("Select Local File for %1").arg(item->title()), Ok|Cancel, Ok, false )
    {
...
PodcastEpisode::associateWithLocalFile()
{
    ...
    if( d.exec() == KDialogBase::Accepted )
    {
        if( !d.url().isLocalFile() || !QFileInfo( d.url().path() ).isFile() )
            Pana::StatusBar::instance()->shortMessage( i18n( "Invalid local podcast URL." ) );
        else
            setLocalUrl( d.url() );
    }
}
...
void SmartPlaylist::showContextMenu( const QPoint &position )
{
    ...
    menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "&Load" ), LOAD );
    menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), ADD );
    menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Tracks" ), QUEUE );
    if( MediaBrowser::isAvailable() )
    {
        menu.insertSeparator();
...
void ShoutcastBrowser::setOpen( bool open )
{
    ...
    tmpfile += "/pana-genres-" + KApplication::randomString(10) + ".xml-";
...
{
    QString tmpfile = tmppath + "/pana-list-" + genre + "-" + KApplication::randomString(10) + ".xml";
