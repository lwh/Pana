...
bool MasEngine::init()
{
    ...
    if (!masinit() ) {
        KMessageBox::error( 0, i18n("<h3>Pana could not initialise MAS.</h3>"
                        "<p>Check for a running mas daemon.</p>") );
        error() << "  connecting to MAS daemon failed. Aborting. " << endl;
        debug() << "  Please restart pana." << endl;
...
bool MasEngine::masinit()
{
    ...
    masc_log_message( 0, (char*)"pana/MASengine" );
