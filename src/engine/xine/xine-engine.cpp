...
//these files are from libpana
#include "playlist.h"
#include "enginecontroller.h"

PANA_EXPORT_PLUGIN( XineEngine )
...
//static inline QCString configPath() { return QFile::encodeName(KStandardDirs().localkdedir() + KStandardDirs::kde_default("data") + "pana/xine-config"); }
static inline QCString configPath() { return QFile::encodeName(locate( "data", "pana/") + "xine-config" ); }
...
XineEngine::load( const KURL &url, bool isStream )
{
    ...
    if( m_xfadeLength > 0 && xine_get_status( m_stream ) == XINE_STATUS_PLAY &&
        ...
        ( m_xfadeNextTrack || //set by engine controller when switching tracks automatically
         (uint) PanaConfig::crossfadeType() == 0 ||  //crossfade always
         (uint) PanaConfig::crossfadeType() == 2 ) ) //crossfade when switching tracks manually
    {
    ...
    switch (xine_get_error( m_stream )) {
        ...
        default:
            if (!xine_get_stream_info( m_stream, XINE_STREAM_INFO_AUDIO_HANDLED ))
            {
                ...
                QString const ext = Pana::extension( m_url.url() ).lower();
...
XineEngine::stop()
{
    ...
    if( PanaConfig::fadeout() && !m_fadeOutRunning || state() == Engine::Paused )
    {
        s_outfader = new OutFader( this, PanaConfig::fadeoutLength() );
        s_outfader->start();
...
Fader::run()
{
    ...
    while ( mix < 1.0 )
    {
        ...
        // get volume (pana main * equalizer preamp)
