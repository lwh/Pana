...
PANA_EXPORT_PLUGIN( HelixEngine )
...
void HelixEngine::interruptUser(unsigned long code, const char *moreinfo, const char *moreinfourl)
{
   ...
   // since this is a serious error, emit trackEnded so pana knows to move on
...
HelixEngine::stop()
{
   ...
   if( PanaConfig::fadeout() && !m_pfade[m_current].m_fadeactive && state() == Engine::Playing ) 
   {
      debug() << "fading out...\n";
      m_state = Engine::Empty;
      ...
      m_pfade[m_current].m_startfadetime = PlayerControl::where(m_current);
      setFadeout(true, PanaConfig::fadeoutLength(), m_current);
   }
   else
   {
...
HelixEngine::timerEvent( QTimerEvent * )
{
   ...
   if ( m_xfadeLength <= 0 && m_state == Engine::Playing && PlayerControl::done(m_current) )
      play_finished(m_current);
   else if ( m_xfadeLength > 0 || PanaConfig::fadeout() )
   {
      ...
      if ( m_pfade[m_current].m_stopfade && m_pfade[m_current].m_fadeactive &&
          (PlayerControl::where(m_current) > m_pfade[m_current].m_startfadetime + (unsigned)PanaConfig::fadeoutLength() ||
