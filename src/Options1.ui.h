...
{
    slotUpdateMoodFrame();
    //kcfg_CustomRatingsColors->setChecked( PanaConfig::customRatingsColors() );
    ...
    kLineEdit_customBrowser->setText( PanaConfig::externalBrowser() );
    int index = browsers.findIndex( PanaConfig::externalBrowser() );
    if( index >= 0 )
        kComboBox_browser->setCurrentItem( index );
    else if( PanaConfig::externalBrowser() ==
#ifdef Q_WS_MAC
            "open"
#else
            "kfmclient openURL"
...
void Options1::slotUpdateRatingsFrame()
{
    ...
    PanaConfig::setCustomRatingsColors( enableStars );
...
void Options1::slotFixedHalfStarColor()
{
    ...
    PanaConfig::setFixedHalfStarColor( kcfg_FixedHalfStarColor->isChecked() );
    slotUpdateRatingsFrame();
}
...
void Options1::slotPickColorHalf()
{
    ...
    {
        PanaConfig::setStarColorHalf( halfStar );
...
void Options1::slotPickColorOne()
{
    ...
    if( result == KColorDialog::Accepted )
    {
        PanaConfig::setStarColorOne( oneStar );
...
void Options1::slotPickColorTwo()
{
    ...
    if( result == KColorDialog::Accepted )
    {
        PanaConfig::setStarColorTwo( twoStar );
...
void Options1::slotPickColorThree()
{
    ...
    if( result == KColorDialog::Accepted )
    {
        PanaConfig::setStarColorThree( threeStar );
...
void Options1::slotPickColorFour()
{
    ...
    if( result == KColorDialog::Accepted )
    {
        PanaConfig::setStarColorFour( fourStar );
...
void Options1::slotPickColorFive()
{
    ...
    if( result == KColorDialog::Accepted )
    {
        PanaConfig::setStarColorFive( fiveStar );
