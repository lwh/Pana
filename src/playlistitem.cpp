 ...
 * copyright (C) 2005 by Alexandre Oliveira aleprj@gmail.com
 *
 *
 * For license please see top level COPYING file
 *
 ***************************************************************************/
...
void PlaylistItem::paintCell( QPainter *painter, const QColorGroup &cg, int column, int width, int align )
{
    ...
    static const QImage currentTrackLeft  = locate( "data", "pana/images/currenttrack_bar_left.png" );
    static const QImage currentTrackMid   = locate( "data", "pana/images/currenttrack_bar_mid.png" );
    static const QImage currentTrackRight = locate( "data", "pana/images/currenttrack_bar_right.png" );
...
//                  paint.setPen( glowText );
    ...
    else
    {
        ...
        else
        {
            ...
            p.setFont( font );
            p.setPen( ( m_isNew && isEnabled() && !isSelected() ) ? PanaConfig::newPlaylistItemsColor() : textc );
    ...
    const bool repeat      = Pana::repeatTrack() && isCurrent;

    const uint num = ( queue ? 1 : 0 ) + ( stop ? 1 : 0 ) + ( repeat ? 1 : 0 );

    static const QPixmap pixstop   = Pana::getPNG(  "currenttrack_stop_small"  ),
                         pixrepeat = Pana::getPNG( "currenttrack_repeat_small" );
    ...
    if( column == listView()->m_firstColumn && num )
    {
        ...
        if( queue )
        {
            ...
            //const QFont smallFont( "Sans", (playNext > 9) ? 9 : 12 );
            //p->setFont( smallFont );
...
{
    // In theory, if PanaConfig::showMoodbar() == false, then the
...
{
    switch( PanaConfig::favorTracks() )
    {
        case PanaConfig::EnumFavorTracks::Off: return 0;
        case PanaConfig::EnumFavorTracks::HigherScores: return score() > 0.f ? static_cast<int>( score() ) : 50;
        case PanaConfig::EnumFavorTracks::HigherRatings: return rating() ? rating() : 5; // 2.5
        case PanaConfig::EnumFavorTracks::LessRecentlyPlayed:
        {
            if( lastPlay() )
                ...
                return listView()->m_startupTime_t - 1058652000; //july 20, 2003, when Pana was first released.
        }
        default: return 0;
    }
}
