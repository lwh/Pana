...
void MagnatunePurchaseHandler::processPayment( QString ccNumber, QString expYear, QString expMonth, QString name, QString email, QString albumCode, int amount )
{
    ...
    amountString.setNum( amount, 10 );
		...
		// PANAFIX - erased the URLs, remove the buying option altogether later
		// Maybe link to the website page and use default browser to open?
		//
    QString purchaseURL = "https://magnatune.com/";
    QString debugPurchaseURL = "https://magnatune.com/";
    debug() << "purchase url : " << debugPurchaseURL << endl;

    m_resultDownloadJob = KIO::storedGet( KURL( purchaseURL ), false, false );
    Pana::StatusBar::instance() ->newProgressOperation( m_resultDownloadJob ).setDescription( i18n( "Processing Payment" ) );
...
void MagnatunePurchaseHandler::saveDownloadInfo( QString infoXml )
{
    ...
    QDir purchaseDir( Pana::saveLocation( "magnatune.com/purchases/" ) );
