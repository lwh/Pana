...
bool MagnatuneBrowser::updateMagnatuneList()
{
    ...
    Pana::StatusBar::instance() ->newProgressOperation( m_listDownloadJob )
...
void MagnatuneBrowser::listDownloadCancelled( )
{
    ...
    Pana::StatusBar::instance() ->endProgressOperation( m_listDownloadJob );
...
void MagnatuneBrowser::polish( )
{
    ...
    if (!m_polished) {
        m_polished = true;
        updateList( );
        m_artistInfobox->begin( KURL( locate( "data", "pana/data/" ) ) );
        m_artistInfobox->write( "<table align='center' border='0'><tbody align='center' valign='top'>"
          ...
          "</div></td></tr><tr><td><BR>"
        + i18n( "Welcome to Pana's BROKEN integrated Magnatune.com store. If this is the "
                "first time you run it, you must update the database by pressing the "
                "'Update' button below. But don't bother , the store doesn't work anymore." )
        + "</td></tr></tbody></table>" );
        m_artistInfobox->end();
