...
void Scrobbler::similarArtists( const QString & artist )
{
    ...
    if ( PanaConfig::retrieveSimilarArtists() )
    {
...
{
    m_submitter->configure( PanaConfig::scrobblerUsername(), PanaConfig::scrobblerPassword(), PanaConfig::submitPlayedSongs() );
}
...
QString ScrobblerSubmitter::CLIENT_ID = "pan";
...
//         << endl << m_submitResultBuffer << endl;
    ...
    {
        warning() << "A new version of Pana is available" << endl;
...
void ScrobblerSubmitter::announceSubmit( SubmitItem *item, int tracks, bool success ) const
{
    ...
    Pana::StatusBar::instance()->shortLongMessage( _short, _long );
}
...
{
    m_savePath = Pana::saveLocation() + "submit.xml";
    QFile file( m_savePath );
