...
#include <qstyle.h>        //Pana::Splitter
#include <qtooltip.h>
...
BrowserBar::BrowserBar( QWidget *parent )
        ...
        , m_divider( new Pana::Splitter( this ) )
        , m_browserBox( new QVBox( this ) )
        , m_currentIndex( -1 )
        , m_lastIndex( -1 )
        , m_mapper( new QSignalMapper( this ) )
{
    m_tabManagementButton = new QPushButton( SmallIconSet(Pana::icon( "configure" )), 0, this, "tab_managment_button" );
    ...
    m_tabBar->setStyle( MultiTabBar::PANA );
    ...
    //m_tabBar->appendButton( Pana::icon( "configure" ), 1, 0, QString::null );

}

BrowserBar::~BrowserBar()
{
    KConfig* const config = Pana::config( "BrowserBar" );
...
{
    const int index = indexForName( Pana::config( "BrowserBar" )->readEntry( "CurrentPane" ) );
    const int width = Pana::config( "BrowserBar" )->readNumEntry( "Width", browser( index )->sizeHint().width() );
...
{
    if( !PanaConfig::autoShowContextBrowser() || m_currentIndex == -1 )
        return;
