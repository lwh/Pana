...
#include <kcalendarsystem.h>  // for Pana::verboseTimeSince()
...
using Pana::QStringx;
using Pana::escapeHTML;
using Pana::escapeHTMLAttr;
using Pana::unescapeHTMLAttr;
...
{
    s_instance = this;
    s_wikiLocale = PanaConfig::wikipediaLocale();
    ...
    m_lyricsToolBar->insertButton( Pana::icon( "refresh" ), LYRICS_REFRESH, true, i18n("Refresh") );
    m_lyricsToolBar->insertButton( Pana::icon( "add_lyrics" ), LYRICS_ADD, true, i18n("Add") );
    m_lyricsToolBar->insertButton( Pana::icon( "edit" ), LYRICS_EDIT, true, i18n("Edit") );
    m_lyricsToolBar->setToggle( LYRICS_EDIT, true );
    m_lyricsToolBar->insertButton( Pana::icon( "search" ), LYRICS_SEARCH, true, i18n("Search") );
    m_lyricsToolBar->setIconText( KToolBar::IconOnly, false );
    m_lyricsToolBar->insertButton( Pana::icon( "external" ), LYRICS_BROWSER, true, i18n("Open in external browser") );
    ...
    { //Search text inside lyrics. Code inspired/copied from playlistwindow.cpp
       ...
       Pana::actionCollection()->setAutoConnectShortcuts ( true );
       new KAction( i18n("Search text in lyrics"), KShortcut("/"), this,SLOT( lyricsSearchTextShow() ), Pana::actionCollection(), "search_text_lyric");
       Pana::actionCollection()->setAutoConnectShortcuts ( false );
    }
    ...
    m_wikiToolBar->insertButton( Pana::icon( "artist" ), WIKI_ARTIST, false, i18n("Artist Page") );
    m_wikiToolBar->insertButton( Pana::icon( "album" ), WIKI_ALBUM, false, i18n("Album Page") );
    m_wikiToolBar->insertButton( Pana::icon( "track" ), WIKI_TITLE, false, i18n("Title Page") );
    m_wikiToolBar->insertLineSeparator();
    m_wikiToolBar->insertButton( Pana::icon( "external" ), WIKI_BROWSER, true, i18n("Open in external browser") );
    m_wikiToolBar->insertButton( Pana::icon( "change_language" ), WIKI_CONFIG, true, i18n("Change Locale") );
    ...
    addTab( m_contextTab, SmallIconSet( Pana::icon( "music" ) ),  i18n( "Music" ) );
    addTab( m_lyricsTab,  SmallIconSet( Pana::icon( "lyrics" ) ), i18n( "Lyrics" ) );
    addTab( m_wikiTab,    SmallIconSet( Pana::icon( "artist" ) ), i18n( "Artist" ) );

    setTabEnabled( m_lyricsTab, false );
    setTabEnabled( m_wikiTab, false );

    m_showRelated   = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowRelated", true );
    m_showSuggested = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowSuggested", true );
    m_showFaves     = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowFaves", true );
    m_showLabels    = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowLabels", true );

    m_showFreshPodcasts  = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowFreshPodcasts", true );
    m_showNewestAlbums   = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowNewestAlbums", true );
    m_showFavoriteAlbums = Pana::config( "ContextBrowser" )->readBoolEntry( "ShowFavoriteAlbums", true );

    // Delete folder with the cached coverimage shadow pixmaps
    KIO::del( KURL::fromPathOrURL( Pana::saveLocation( "covershadow-cache/" ) ), false, false );
...
//     setMinimumHeight( PanaConfig::coverPreviewSize() + (fontMetrics().height()+2)*5 + tabBar()->height() );
}
...
void ContextBrowser::openURLRequest( const KURL &url )
{
    ...
    Pana::albumArtistTrackFromUrl( url.path(), artist, album, track );
    ...
    if ( url.protocol() == "http" )
    {
        if ( url.hasHTMLRef() )
        {
            ...
            // Wikipedia has links to other pages with anchors, so we have to check if its for the current one
    ...
    else if ( url.protocol() == "musicbrainz" )
    {
        ...
        Pana::invokeBrowser( url.arg( KURL::encode_string_no_slash( artist, 106 /*utf-8*/ ),
        KURL::encode_string_no_slash( album, 106 /*utf-8*/ ),
        KURL::encode_string_no_slash( track, 106 /*utf-8*/ ) ) );
    }

    else if ( url.protocol() == "externalurl" )
        Pana::invokeBrowser( url.url().replace( QRegExp( "^externalurl:" ), "http:") );
...
void ContextBrowser::engineNewMetaData( const MetaBundle& bundle, bool trackChanged )
{
    ...
    {
        newMetaData = true;
        const QString timeString = KGlobal::locale()->formatTime( QTime::currentTime() ).replace(" ", "&nbsp;"); // don't break long lines
...
{
    QFile exportedDocument( Pana::saveLocation() + "contextbrowser.html" );
...
void ContextBrowser::slotContextMenu( const QString& urlString, const QPoint& point )
{
    ...
    {
        Pana::coverContextMenu( this, point, artist, album );
        return;
    }
    else if( url.protocol() == "stream" )
    {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "&Load" ), MAKE );
        menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), APPEND );
        menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Podcast" ), ASNEXT );
    ...
    else if( url.protocol() == "file" || url.protocol() == "artist" || url.protocol() == "album" || url.protocol() == "compilation" || url.protocol() == "albumdisc" || url.protocol() == "compilationdisc")
    {
        ...
        menu.insertItem( SmallIconSet( Pana::icon( "files" ) ), i18n( "&Load" ), MAKE );
        menu.insertItem( SmallIconSet( Pana::icon( "add_playlist" ) ), i18n( "&Append to Playlist" ), APPEND );
        menu.insertItem( SmallIconSet( Pana::icon( "queue_track" ) ), i18n( "&Queue Track" ), ASNEXT );
        if( MediaBrowser::isAvailable() )
            menu.insertItem( SmallIconSet( Pana::icon( "device" ) ), i18n( "&Transfer to Media Device" ), MEDIA_DEVICE );

        menu.insertSeparator();
        menu.insertItem( SmallIconSet( Pana::icon( "info" ) ), i18n( "Edit Track &Information..." ), INFO );
    ...
    case RELATED:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowRelated", m_showRelated );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
    ...
    case SUGGEST:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowSuggested", m_showSuggested );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
    ...
    case FAVES:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowFaves", m_showFaves );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
   ...
   case LABELS:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowLabels", m_showLabels );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
   ...
   case FRESHPODCASTS:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowFreshPodcasts", m_showFreshPodcasts );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
   ...
   case NEWALBUMS:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowNewestAlbums", m_showNewestAlbums );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
   ...
   case FAVALBUMS:
        ...
        Pana::config( "ContextBrowser" )->writeEntry( "ShowFavoriteAlbums", m_showFavoriteAlbums );
        m_dirtyCurrentTrackPage = true;
        showCurrentTrack();
        break;
...
public:
        ...
        , m_isStream( EngineController::engine()->isStream() )
    {
        ...
        m_panaIconPath = QDeepCopy<QString>(KGlobal::iconLoader()->iconPath( "pana",
                    -KIcon::SizeEnormous ) );
        m_musicBrainIconPath = QDeepCopy<QString>(locate( "data", "pana/images/musicbrainz.png" )
                );
        m_lastfmIcon = "file://" + locate( "data","pana/images/lastfm.png" );
    }
...
void CurrentTrackJob::showHome()
{
    ...
    m_HTMLSource.append(
            QStringx(
                ...
                "<a href='%1'><img id='current_box-largecover-image' src='%2' title='Pana'></a>\n"
                "</td>\n"
                ...
                "</div>\n" )
            .args( QStringList()
                    << escapeHTMLAttr( "externalurl://pana.bunnies.net" )
                    << escapeHTMLAttr( m_panaIconPath )
...
CurrentTrackJob::showHomeByAlbums()
{
    ...
    if( ContextBrowser::instance()->m_showFavoriteAlbums )
    {
        ...
        bool ratings = PanaConfig::useRatings();
        bool scores = PanaConfig::useScores();
...
void CurrentTrackJob::showLastFm( const MetaBundle &currentTrack )
{
    ...
    const QString username   = PanaConfig::scrobblerUsername();
    ...
    const QString skipIcon = KGlobal::iconLoader()->iconPath( Pana::icon("next"),   -KIcon::SizeSmallMedium );
    const QString loveIcon = KGlobal::iconLoader()->iconPath( Pana::icon("love"),   -KIcon::SizeSmallMedium );
    const QString banIcon  = KGlobal::iconLoader()->iconPath( Pana::icon("remove"), -KIcon::SizeSmallMedium );
    ...
    if ( !values.isEmpty() && values[2].toInt() )
    {
        ...
        m_HTMLSource.append( QString(
                ...
                .arg( i18n( "Track played once", "Track played %n times", playtimes ),
                      ...
                      i18n( "Last played: %1" ).arg( Pana::verboseTimeSince( lastPlay ) ),
                      i18n( "First played: %1" ).arg( Pana::verboseTimeSince( firstPlay ) ) ) );
    }
    else
    ...
    m_HTMLSource.append(
            ...
            "<div id='building_box' class='box'>\n"
                ...
                "<div id='building_box-body' class='box-body'>\n"
                    "<div class='info'><p>\n" + i18n( "Please be patient while Pana scans your music collection. You can watch the progress of this activity in the statusbar." ) + "</p></div>\n"
                "</div>\n"
            "</div>\n"
            "</body></html>\n"
                       );
...
{
    Pana::invokeBrowser( m_lyricCurrentUrl );
}
...
{
    Pana::invokeBrowser( m_lyricAddUrl );
}
...
{
    Pana::invokeBrowser( m_lyricSearchUrl );
}
...
{
    PanaConfig::setWikipediaLocale( locale );
    s_wikiLocale = locale;
}
...
void ContextBrowser::showWikipedia( const QString &url, bool fromHistory, bool replaceHistory )
{
    ...
    Pana::StatusBar::instance()->newProgressOperation( m_wikiJob )
...
{
    Pana::invokeBrowser( m_wikiCurrentUrl );
}
...
ContextBrowser::wikiResult( KIO::Job* job ) //SLOT
{
    ...
    // Add the license information back in
    m_wiki += copyright;
    m_wiki.append( "</div>" );
...
void ContextBrowser::ratingOrScoreOrLabelsChanged( const QString &path ) //SLOT
{
    ...
    if( m_browseLabels || ( currentTrack.isFile() && ( currentTrack.url().path() == path || PanaConfig::useRatings() ) ) )
...
ContextBrowser::expandURL( const KURL &url )
{
    ...
    if( protocol == "artist" ) {
        ...
        Pana::albumArtistTrackFromUrl( url.path(), artist, album, discnumber );
