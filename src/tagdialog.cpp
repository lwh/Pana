...
TagDialog::~TagDialog()
{
    Pana::config( "TagDialog" )->writeEntry( "CurrentTab", kTabWidget->currentPageIndex() );
}
...
TagDialog::loadCover( const QString &artist, const QString &album )
{
    ...
    const int s = PanaConfig::coverPreviewSize();
...
void TagDialog::init()
{
    ...
    KConfig *config = Pana::config( "TagDialog" );
...
//    const QStringList genres = MetaBundle::genreList();
    ...
    pushButton_open->setIconSet( SmallIconSet( Pana::icon( "files" ) ) );
...
TagDialog::applyToAllTracks()
{
    ...
    for ( KURL::List::ConstIterator it = m_urlList.begin(); it != end; ++it ) {
        ...
        if( !kComboBox_artist->currentText().isEmpty() && 
		  		kComboBox_artist->currentText() != mb.artist()  ||
				kComboBox_artist->currentText().isEmpty() &&
				!m_bundle.artist().isEmpty() ) {
            mb.setArtist( kComboBox_artist->currentText() );
