...
namespace Pana
{
    ...
    {
        return QImage( locate( "data", QString( "pana/images/b_%1.png" ).arg( iconName ) ), "PNG" ).smoothScale( 10, 10 );
    }
}


Pana::TrayIcon::TrayIcon( QWidget *playerWidget )
        ...
        , m_lastFmMode( false )
{
    ...
    baseIcon     = KSystemTray::loadIcon( "pana" );
    playOverlay  = Pana::loadOverlay( "play" );
    pauseOverlay = Pana::loadOverlay( "pause" );
    overlayVisible = false;
...
Pana::TrayIcon::event( QEvent *e )
{
    ...
    case QEvent::DragEnter:
        return Pana::genericEventHandler( this, e );
...
void
Pana::TrayIcon::engineStateChanged( Engine::State state, Engine::State /*oldState*/ )
{
    // stop timer
    if ( blinkTimerID )
    {
...
void
Pana::TrayIcon::engineNewMetaData( const MetaBundle &bundle, bool /*trackChanged*/ )
{
...
void
Pana::TrayIcon::engineTrackPositionChanged( long position, bool /*userSeek*/ )
{
...
void
Pana::TrayIcon::paletteChange( const QPalette & op )
{
...
void
Pana::TrayIcon::paintIcon( int mergePixels, bool force )
{
...
void
Pana::TrayIcon::blendOverlay( QPixmap &sourcePixmap )
{
...
void
Pana::TrayIcon::setLastFm( bool lastFmActive )
{
