...
MediumPluginManagerDialog::MediumPluginManagerDialog()
        : KDialogBase( Pana::mainWindow(), "mediumpluginmanagerdialog", false, QString::null, Ok|Cancel, Ok )
{
...
MediumPluginManager::redetectDevices()
{
    ...
    {
        Pana::StatusBar::instance()->longMessageThreadSafe( i18n("No new media devices were found. If you feel this is an\n"
...
MediumPluginManager::newDevice()
{
    ...
    {
        if( !Pana::config( "MediaBrowser" )->readEntry( mda->getMedium()->id() ).isNull() )
        {
            //abort!  Can't have the same device defined twice...should never
            //happen due to name checking earlier...right?
            Pana::StatusBar::instance()->longMessageThreadSafe( i18n("Sorry, you cannot define two devices\n"
                                                                       "with the same name and mountpoint!") );
        }
        else
        {
            Medium *newdev = new Medium( mda->getMedium() );
            Pana::config( "MediaBrowser" )->writeEntry( newdev->id(), mda->getPlugin() );
...
: KDialogBase( Pana::mainWindow(), "manualdeviceadder", true, QString::null, Ok|Cancel, Ok )
{
    m_mpm = mpm;
    m_successful = false;
    m_newMed = 0;
...
ManualDeviceAdder::slotOk()
{
    ...
    {
        Pana::StatusBar::instance()->longMessageThreadSafe( i18n("Sorry, every device must have a name and\n"
...
, m_new( true )
{
    ...
    (void)new KActiveLabel( i18n( "(<a href='whatsthis:%1'>Details</a>)" )
                            .arg( Pana::escapeHTMLAttr( details ) ), this );
    ...
    for( KTrader::OfferList::ConstIterator it = MediaBrowser::instance()->getPlugins().begin();
        ...
        if ( (*it)->property( "X-KDE-Pana-name" ).toString() == config->readEntry( medium->id() ) )
            m_pluginCombo->setCurrentItem( (*it)->name() );
    }

    m_configButton = new KPushButton( SmallIconSet( Pana::icon( "configure" ) ), QString::null, this );
...
{
    //TODO:save something in panarc such that it's not shown again until hit autoscan
    //m_deletedMap[medium->id()] = medium;
