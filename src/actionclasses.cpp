...
namespace Pana
{
    bool repeatNone() { return PanaConfig::repeat() == PanaConfig::EnumRepeat::Off; }
    bool repeatTrack() { return PanaConfig::repeat() == PanaConfig::EnumRepeat::Track; }
    bool repeatAlbum() { return PanaConfig::repeat() == PanaConfig::EnumRepeat::Album; }
    bool repeatPlaylist() { return PanaConfig::repeat() == PanaConfig::EnumRepeat::Playlist; }
    bool randomOff() { return PanaConfig::randomMode() == PanaConfig::EnumRandomMode::Off; }
    bool randomTracks() { return PanaConfig::randomMode() == PanaConfig::EnumRandomMode::Tracks; }
    bool randomAlbums() { return PanaConfig::randomMode() == PanaConfig::EnumRandomMode::Albums; }
    bool favorNone() { return PanaConfig::favorTracks() == PanaConfig::EnumFavorTracks::Off; }
    bool favorScores() { return PanaConfig::favorTracks() == PanaConfig::EnumFavorTracks::HigherScores; }
    bool favorRatings() { return PanaConfig::favorTracks() == PanaConfig::EnumFavorTracks::HigherRatings; }
    bool favorLastPlay() { return PanaConfig::favorTracks() == PanaConfig::EnumFavorTracks::LessRecentlyPlayed; }
    bool entireAlbums() { return repeatAlbum() || randomAlbums(); }
}

using namespace Pana;
...
MenuAction::MenuAction( KActionCollection *ac )
  : KAction( i18n( "Pana Menu" ), 0, ac, "pana_menu" )
{
...
MenuAction::plug( QWidget *w, int index )
{
    ...
    if( bar && kapp->authorizeKAction( name() ) )
    {
        ...
        button->setPopup( Pana::Menu::instance() );
        button->setName( "toolbutton_pana_menu" );
        button->setIcon( "pana" );
...
Menu::Menu()
{
    ...
    insertItem( SmallIconSet( Pana::icon( "covermanager" ) ), i18n( "C&over Manager" ), ID_SHOW_COVER_MANAGER );
    safePlug( ac, "queue_manager", this );
    insertItem( SmallIconSet( Pana::icon( "visualizations" ) ), i18n( "&Visualizations" ), ID_SHOW_VIS_SELECTOR );
    insertItem( SmallIconSet( Pana::icon( "equalizer" ) ), i18n( "E&qualizer" ), kapp, SLOT( slotConfigEqualizer() ), 0, ID_CONFIGURE_EQUALIZER );
    ...
    insertItem( SmallIconSet( Pana::icon( "rescan" ) ), i18n("&Rescan Collection"), ID_RESCAN_COLLECTION );
...
Menu::helpMenu( QWidget *parent ) //STATIC
{
    ...
    if ( s_helpMenu == 0 )
        s_helpMenu = new KHelpMenu( parent, &aboutData, Pana::actionCollection() );
...
AnalyzerContainer::mousePressEvent( QMouseEvent *e)
{
    if( e->button() == Qt::LeftButton ) {
        PanaConfig::setCurrentPlaylistAnalyzer( PanaConfig::currentPlaylistAnalyzer() + 1 );
        changeAnalyzer();
    }
}
...
AnalyzerContainer::contextMenuEvent( QContextMenuEvent *e)
{
    ...
    KPopupMenu menu;
    menu.insertItem( SmallIconSet( Pana::icon( "visualizations" ) ), i18n("&Visualizations"), Menu::ID_SHOW_VIS_SELECTOR );
...
void ToggleAction::setChecked( bool b )
{
    ...
    PanaConfig::writeConfig(); //So we don't lose the setting when crashing
...
VolumeAction::plug( QWidget *w, int index )
{
    ...
    delete static_cast<Pana::VolumeSlider*>( m_slider ); //just in case, remember, we only support one plugging!

    m_slider = new Pana::VolumeSlider( w, Pana::VOLUME_MAX );
    m_slider->setName( "ToolBarVolume" );
    m_slider->setValue( PanaConfig::masterVolume() );
...
RandomAction::RandomAction( KActionCollection *ac ) :
    SelectAction( i18n( "Ra&ndom" ), &PanaConfig::setRandomMode, ac, "random_mode" )
{
    setItems( QStringList() << i18n( "&Off" ) << i18n( "&Tracks" ) << i18n( "&Albums" ) );
    setCurrentItem( PanaConfig::randomMode() );
    setIcons( QStringList() << Pana::icon( "random_no" ) << Pana::icon( "random_track" ) << Pana::icon( "random_album" ) );
}
...
FavorAction::FavorAction( KActionCollection *ac ) :
    SelectAction( i18n( "&Favor" ), &PanaConfig::setFavorTracks, ac, "favor_tracks" )
{
    ...
    setCurrentItem( PanaConfig::favorTracks() );
    setEnabled( PanaConfig::randomMode() );
}
...
RepeatAction::RepeatAction( KActionCollection *ac ) :
    SelectAction( i18n( "&Repeat" ), &PanaConfig::setRepeat, ac, "repeat" )
{
    setItems( QStringList() << i18n( "&Off" ) << i18n( "&Track" )
                            << i18n( "&Album" ) << i18n( "&Playlist" ) );
    setIcons( QStringList() << Pana::icon( "repeat_no" ) << Pana::icon( "repeat_track" ) << Pana::icon( "repeat_album" ) << Pana::icon( "repeat_playlist" ) );
    setCurrentItem( PanaConfig::repeat() );
}
...
BurnMenuAction::plug( QWidget *w, int index )
{
    ...
    if( bar && kapp->authorizeKAction( name() ) )
    {
        ...
        KToolBarButton* button = bar->getButton( id );
        button->setPopup( Pana::BurnMenu::instance() );
...
StopAction::StopAction( KActionCollection *ac )
  : KAction( i18n( "Stop" ), Pana::icon( "stop" ), 0, EngineController::instance(), SLOT( stop() ), ac, "stop" )
{}
...
StopAction::plug( QWidget *w, int index )
{
    ...
    if( bar && kapp->authorizeKAction( name() ) )
    {
        ...
        KToolBarButton* button = bar->getButton( id );
        button->setDelayedPopup( Pana::StopMenu::instance() );
        button->setName( "toolbutton_stop_menu" );
        button->setIcon( Pana::icon( "stop" ) );
...
StopMenu::slotAboutToShow()
{
    ...
    setItemEnabled( NOW,         Pana::actionCollection()->action( "stop" )->isEnabled() );
...
StopMenu::slotActivated( int index )
{
    ...
    {
        case NOW:
            Pana::actionCollection()->action( "stop" )->activate();
